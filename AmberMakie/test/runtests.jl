...
@testset "engineering formatting" begin
    ...
    @test engineering(-8.0e-12; unit = "A") == "-8.0 pA"
...
@testset "statistical adapters and plots" begin
    sweep = Amber.SweepResult(
        "R.value", Any[1.0, 2.0, 3.0], Any[2.0, nothing, 6.0],
    ...
    grid_monte = Amber.MonteCarloResult(
        Amber.OperatingPoint(),
    ...
    yield_map = yieldmapplot(
        Makie.Figure()[1, 1], grid_monte;
    ...
    outlier_result = Amber.MonteCarloResult(
        Amber.OperatingPoint(),
    ...
    outlier_correlation = correlationplot(
        Makie.Figure()[1, 1], outlier_result;
        parameter = :x
    )
    ...
    replay_result = monte_carlo(
        replay_circuit; samples = 3, seed = 7,
        variations = Dict(Symbol("R1.value") => Gaussian(1kΩ, 20Ω)),
        metric = replay_metric
    )
    replay_workbench = workbench(
        replay_result; circuit = replay_circuit,
        metric = replay_metric
    )
...
@testset "network and control integration" begin
    @circuit MakieSmallSignalWorkbench() begin
        ...
        R1 = resistor(input, output; value = 1kΩ)
        C1 = capacitor(output, gnd; value = 1.0e-6)
    end
    ac = small_signal(
        MakieSmallSignalWorkbench(), [1.0, 10.0, 100.0];
        source = :V1
    )
    ac_workbench = workbench(
        ac; input = voltage(:input), output = voltage(:output),
    ...
    @test ac_workbench.measurements[:frequency_readout][].a.gain_db ≈ 0.0 atol = 1.0e-10
    ...
    result = port_response(
        MakieTwoPort(), [10Hz, 1kHz];
    ...
    impedance = impedanceplot(
        Makie.Figure()[1, 1], result;
...
@testset "spectrum adapter" begin
    result = SpectrumResult(
        [0.0, 1.0], ComplexF64[0, 1], [0.0, 0.5], [0.0, 0.25],
...
@testset "spectrogram and noise detail plots" begin
    ...
    spectrogram = spectrogramview(
        transient_result; signal = :output, samples = 64,
    ...
    @test hasproperty(
        spectrogramplot(
            Makie.Figure()[1, 1], transient_result;
            signal = :output, samples = 64
        ).axes, :spectrogram
    )
    eye = eyediagramview(
        transient_result; signal = :output, period = 1.0e-3,
    ...
    @test hasproperty(
        eyediagramplot(
            Makie.Figure()[1, 1], transient_result;
            signal = :output, period = 1.0e-3
        ).axes, :eye
    )
    jitter = jitterview(transient_result; signal = :output, nominal_period = 1.0e-3)
    @test length(jitter.crossings) >= 3
    @test maximum(abs, jitter.period_jitter) < 1.0e-5
    bathtub = jitterplot(
        Makie.Figure()[1, 1], transient_result;
        signal = :output, nominal_period = 1.0e-3, kind = :bathtub
    )
    @test bathtub.view.kind === :bathtub
    @test all(>(0), bathtub.view.display.y)
    comparison_result = transient(
        MakieSignal(), 0.1ms => 3.9ms;
        initial = :discharged, saveat = 25μs, method = :bdf2
    )
    comparison = compareplot(
        Makie.Figure()[1, 1],
    ...
    pss = Amber.PSSResult(
        transient_result, 4.0e-3, 1.0e-8, 3,
        begin
    ...
    close(pss_workbench)
    harmonic_result = harmonic_analysis(
        transient_result; signal = :output,
        fundamental = 1.0e3, harmonics = 3
    )
    ...
    @test any(
        row -> row.name == "R1" && row.power !== nothing,
    ...
    power_handle = powerdashboard(
        Makie.Figure()[1, 1], operating_point_result;
        output = :R2
    )
    @test power_handle.view.supplied > 0
    @test 0 <= power_handle.view.efficiency <= 1
    noise_result = noise(
        MakieNoise(), [10Hz, 100Hz, 1kHz, 10kHz];
    ...
    @test hasproperty(
        noisecontributionplot(Makie.Figure()[1, 1], noise_result).axes,
        :noise_contributions
    )
    budget = noisebudgetplot(
        Makie.Figure()[1, 1], noise_result;
        group = :component
    )
    ...
    phase_ratio = fill(1.0e-12, length(noise_result.frequencies))
    phase_result = Amber.PhaseNoiseResult(
        noise_result.frequencies, noise_result.output,
        phase_ratio, 10log10.(phase_ratio), 1.0e-6, noise_result.output_psd,
    ...
    @test hasproperty(
        diagnosticplot(Makie.Figure()[1, 1], phase_result).plots,
        :device_validity
    )
    phase_handle = phasenoiseplot(
        Makie.Figure()[1, 1], phase_result;
        carrier_frequency = 1.0e6
    )
    @test phase_handle.view.integrated > 0
    @test phase_handle.view.carrier_frequency == 1.0e6
    phase_workbench = workbench(phase_result; carrier_frequency = 1.0e6)
    ...
    sidebands = [-1, 0, 1]
    periodic_result = Amber.PeriodicNoiseResult(
        noise_result.frequencies,
        noise_result.output, nothing, 1, sidebands, noise_result.output_psd,
        nothing, fill(1.0e-12, length(sidebands), length(noise_result.frequencies)),
        noise_result.contributions, noise_result.compiled,
        Dict{Symbol, Any}(:warnings => String[])
    )
    @test hasproperty(
        periodicnoiseplot(Makie.Figure()[1, 1], periodic_result).axes,
        :periodic_noise
    )
...
@testset "workbench ownership" begin
    result = SpectrumResult(
        [0.0, 1.0, 2.0], ComplexF64[0, 1, 0], [0.0, 0.5, 0.0],
