@testset "CMOS characterization and rendering" begin
    model = ChargeBasedMOSFET(width = 8μm, length = 2μm, channel_length_modulation = 0.02)
    view = mosfetview(model; vgs = range(0.1, 1.4; length = 31), vds = [0.05, 0.6, 1.2])
    @test size(view.points) == (31, 3)
    @test view.points[12, 2] == mosfet_operating_point(model, :nmos, 0.6, view.vgs[12], 0.0, 0.0)
    ...
    @test_throws ArgumentError mosfetview(model; vgs = [1.0, 0.0])
    @test_throws ArgumentError mosfetview(model; vds = [])
    @test_throws ArgumentError mosfetview(model; vgs = [0.0, NaN])
    directory = mktempdir()
    for (name, handle) in (
            ("transfer", mosfetplot(Figure()[1, 1], view; scale = :log)),
            ("gmid", gmidplot(Figure()[1, 1], pview)),
            ("capacitance", capacitanceplot(Figure()[1, 1], view.points[12, 2])),
            (
                "output", mosfetplot(
                    Figure()[1, 1], mosfetview(
                        model;
                        vgs = [0.6, 0.8, 1.0, 1.2], vds = range(0.0, 1.8; length = 41)
                    ); x = :vds
                ),
            ),
        )
        ...
        if name == "capacitance"
            ...
            @test handle.plots.capacitance[3][][1, 2] ≈ view.points[12, 2].capacitance_matrix[2, 1] * 1.0e15
        end
    end
    h = workbench(view)
...
@testset "plot regression cases" begin
    ...
    @test h.cursors.interval_readout[].rms ≈ interval_readout(tr.axis, voltage(tr, :output), 0.0001 => 0.001).rms
    selectsignal!(h, current(:R))
    setcursor!(h.cursors, :a, 0.0005)
    @test h.cursors.readout[].a.y == nearest_sample(tr.axis, current(tr, :R), 0.0005).y
    @test occursin("A", h.measurements[:interval_text][])
    @test h.axes.trace.ylabel[] == "A"
    @test maximum(abs, h.axes.trace.limits[][2]) < 0.01
    showalltraces!(h)
    ...
    @test h.cursors.readout[].a.y == nearest_sample(tr.axis, voltage(tr, :output), 0.0005).y
    ...
    sweep_result = Amber.SweepResult(
        "bias", Any[0.0, 1.0, 2.0], Any[0.0, nothing, 2.0],
        Any[nothing, nothing, nothing], BitVector([true, false, true]),
        [Amber.SweepFailure(2, 1.0, :ConvergenceError, "failed")], Amber.OperatingPoint(), Dict{Symbol, Any}()
    )
    sh = sweepplot(Figure()[1, 1], sweep_result)
    @test any(p -> isnan(p[2]), sh.plots.curve[1][])
    @test AmberMakie._finite_derivative([0.0, 1.0, 3.0], [0.0, 1.0, 9.0])[2] ≈ 2.0
    @circuit QuietResistor() begin
        gnd = ground(); output = node()
        R = resistor(output, gnd; value = 1.0)
    end
    nr = noise(QuietResistor(), [10.0, 100.0, 1000.0]; output = voltage(:output))
    ...
    @circuit ClampedResistor() begin
        gnd = ground(); output = node()
        R = resistor(output, gnd; value = 1.0)
        V = voltage_source(output, gnd; dc = 0.0)
    end
    zero_noise = noise(ClampedResistor(), [10.0, 100.0, 1000.0]; output = voltage(:output))
