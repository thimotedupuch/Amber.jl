...
mos_view = mosfetview(
    ...
    ); vgs = range(0.0, 1.5; length = 151),
    vds = [0.05, 0.25, 1.2]
)
network_result = port_response(
    ReadmeTwoPort(), 1MHz => 10GHz; points = 320,
...
Label(
    fig[1, 1:2], "AMBER  /  CIRCUIT SIMULATION";
    color = CYAN, fontsize = 22, font = :bold, halign = :left, tellwidth = false
)
Label(
    fig[2, 1:2], "Circuit simulation, fully programmable";
    color = TEXT, fontsize = 47, font = :bold, halign = :left, tellwidth = false
)
Label(
    fig[3, 1:2], "Time domain     ·     Frequency response     ·     Device physics     ·     RF networks";
    color = MUTED, fontsize = 21, halign = :left, tellwidth = false
)

ax_time = Axis(
    fig[4, 1]; title = "01   TRANSIENT  /  RC LOW-PASS",
    xlabel = "Time (ms)", ylabel = "Voltage (V)"
)
times = transient_result.axis .* 1.0e3
...
axislegend(
    ax_time; position = :rt, framevisible = false, labelsize = 19,
    patchsize = (30, 12)
)
xlims!(ax_time, 0, 3)
...
ax_bode = Axis(
    fig[4, 2]; title = "02   SMALL-SIGNAL  /  FILTER RESPONSE",
    xlabel = "Frequency (Hz)", ylabel = "Gain (dB)", xscale = log10,
    xticks = ([10, 100, 1.0e3, 1.0e4, 1.0e5, 1.0e6], ["10", "100", "1k", "10k", "100k", "1M"])
)
...
text!(
    ax_bode, corner * 1.16, -8.0; text = "−3 dB · 1.59 kHz", color = CORAL,
    fontsize = 19, align = (:left, :center)
)
xlims!(ax_bode, 10, 1.0e6)
ylims!(ax_bode, -58, 5)

ax_mos = Axis(
    fig[5, 1]; title = "03   CMOS  /  NMOS BIAS SWEEP",
    xlabel = "Gate voltage VGS (V)", ylabel = "Drain current (μA)", yscale = log10,
    yticks = ([1.0e-3, 1.0e-1, 10, 1.0e3], ["0.001", "0.1", "10", "1,000"])
)
for (j, color) in enumerate((CYAN, LILAC, CORAL))
    current_ua = [max(abs(point.id) * 1.0e6, 1.0e-8) for point in mos_view.points[:, j]]
    lines!(
        ax_mos, mos_view.vgs, current_ua; color, linewidth = 3.2,
        label = "VDS = $(mos_view.vds[j]) V"
    )
end
axislegend(
    ax_mos; position = :lt, framevisible = false, labelsize = 18,
    patchsize = (28, 12)
)
xlims!(ax_mos, 0, 1.5)
ylims!(ax_mos, 1.0e-3, 1.0e3)

ax_rf = Axis(
    fig[5, 2]; title = "04   RF  /  INPUT REFLECTION",
...
for resistance in (0.2, 0.5, 1.0, 2.0, 5.0)
    ...
    lines!(
        ax_rf, center .+ radius .* cos.(θ), radius .* sin.(θ);
        color = (MUTED, 0.2), linewidth = 0.9
    )
end
for reactance in (0.5, 1.0, 2.0), sign in (-1, 1)
    ...
    reflection = (z .- 1) ./ (z .+ 1)
    lines!(
        ax_rf, real.(reflection), imag.(reflection);
...
Label(
    fig[6, 1:2], "Amber.jl  +  AmberMakie     •     Plots from numerical simulations and a charge-based MOSFET model";
