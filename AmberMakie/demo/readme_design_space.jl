...
@circuit EyeLink() begin
    ...
    Bits = voltage_source(
        drive, gnd; dc = 0V,
        waveform = Pulse(
            low = 0V, high = 1V, frequency = 1MHz,
            duty_cycle = 0.48, rise = 20ns, fall = 20ns
        )
    )
    Interference = voltage_source(
        input, drive; dc = 0V,
...
trials = monte_carlo(
    YieldDivider(); samples = 4_000, seed = 2026,
    variations = (
        Symbol("Rtop.value") => UniformVariation(800Ω, 1200Ω),
...
for sample in eachindex(trials.values)
    ...
    top = parameters[Symbol("Rtop.value")] / 1.0e3
    bottom = parameters[Symbol("Rbottom.value")] / 1.0e3
...
resonant_response = small_signal(
    ResonantBandpass(),
...
Label(
    fig[1, 1:2], "AMBER  /  DESIGN EXPLORATION";
    color = CYAN, fontsize = 22, font = :bold, halign = :left, tellwidth = false
)
Label(
    fig[2, 1:2], "From stability to phase space";
    color = TEXT, fontsize = 47, font = :bold, halign = :left, tellwidth = false
)
Label(
    fig[3, 1:2], "Descriptor control     ·     Signal integrity     ·     Statistical yield     ·     Resonant response";
    color = MUTED, fontsize = 21, halign = :left, tellwidth = false
)

ax_locus = Axis(
    fig[4, 1]; title = "01   CONTROL  /  CLOSED-LOOP POLES",
...
for (gain, roots) in zip(gain_values, locus)
    scatter!(
        ax_locus, real.(roots) ./ 1.0e3, imag.(roots) ./ 1.0e3;
...
scatter!(
    ax_locus, real.(first(locus)) ./ 1.0e3,
    imag.(first(locus)) ./ 1.0e3;
    color = CYAN, marker = :x, markersize = 21, strokewidth = 3
)
text!(
    ax_locus, -12.7, 7.3; text = "gain 0 → 60", color = GOLD,
    fontsize = 19
)
text!(
    ax_locus, -12.7, -7.6; text = "stable", color = MUTED,
    fontsize = 18
)
text!(
    ax_locus, 0.2, -7.6; text = "unstable", color = CORAL,
    fontsize = 18
)
...
ax_eye = Axis(
    fig[4, 2]; title = "02   SIGNAL INTEGRITY  /  EYE DIAGRAM",
    xlabel = "Unit intervals", ylabel = "Output voltage (V)"
)
for (phase, values) in zip(eye.phases, eye.values)
    lines!(ax_eye, phase, values; color = (CYAN, 0.3), linewidth = 2.0)
end
hlines!(
    ax_eye, [0.5]; color = (GOLD, 0.6), linewidth = 1.5,
    linestyle = :dash
)
vlines!(
    ax_eye, [0.5, 1.5]; color = (GOLD, 0.4), linewidth = 1.3,
    linestyle = :dash
)
xlims!(ax_eye, 0, 2)
...
ax_yield = Axis(
    yield_layout[1, 1]; title = "03   DESIGN SPACE  /  YIELD MAP",
    xlabel = "Top resistance (kΩ)", ylabel = "Bottom resistance (kΩ)"
)
yield_plot = heatmap!(
    ax_yield, centers, centers, rates;
...
Colorbar(
    yield_layout[1, 2], yield_plot; label = "Yield",
...
hlines!(ax_lissajous, [0]; color = (MUTED, 0.3), linewidth = 1)
vlines!(ax_lissajous, [0]; color = (MUTED, 0.3), linewidth = 1)
for (index, color) in enumerate((CYAN, GOLD, CORAL))
    frequency = lissajous_frequencies[index]
    output = real.(phasors[index] .* exp.(im .* phase))
    lines!(
        ax_lissajous, cos.(phase), output;
        color, linewidth = 3.2, label = "$(frequency / 1kHz) kHz"
    )
end
axislegend(
    ax_lissajous; position = :lt, framevisible = false,
...
Label(
    fig[6, 1:2], "120 feedback gains   •   20 eye folds   •   4,000 circuit trials   •   3 AC phasors";
