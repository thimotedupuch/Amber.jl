...
@circuit DemoLowPass() begin
    ...
    output = node()
    V1 = voltage_source(
        input, gnd; dc = 0V, ac = 1V,
        waveform = Pulse(
            low = 0V, high = 1V, delay = 100μs, rise = 20μs,
...
bodeplot(
    frequency_figure[1, 1], frequency_result;
...
network_result = port_response(
    DemoTwoPort(), 1MHz => 10GHz; points = 320,
...
converged = .!failures
failure_records = [
    MonteCarloFailure(index, UInt64(index), :ConvergenceError, "synthetic corner failure")
        for index in findall(failures)
]
ensemble = MonteCarloResult(
    OperatingPoint(), values, parameters,
...
correlationplot(
    statistics_figure[1, 2], ensemble;
...
function save_demo(name, figure_or_handle)
    ...
    return hasproperty(figure_or_handle, :figure) && close(figure_or_handle)
end
...
jitterplot(
    signal_quality[1, 2], transient_result; signal = :output,
...
jitterplot(
    jitter_figure[1, 1], transient_result; signal = :output,
    nominal_period = 1ms, kind = :tie
)
jitterplot(
    jitter_figure[1, 2], transient_result; signal = :output,
...
spectrogramplot(
    spectrogram_figure[1, 1], transient_result;
...
frequency_handle = workbench(
    frequency_result; input = voltage(:input),
...
impedanceplot(
    network_details[1, 1], network_result; element = (1, 1),
...
noise_result = noise(
    DemoBias(), 10Hz => 1MHz; output = voltage(:output),
...
phase_ratio = max.(noise_result.output_psd ./ maximum(noise_result.output_psd), eps()) .* 1.0e-10
...
pss_result = Amber.PSSResult(
    transient_result, 3ms, 1.0e-8, 4,
...
sweep_metrics = 1.0 ./ (2π .* sweep_values .* 100.0e-9)
sweep_result = Amber.SweepResult(
    "R1.value", Any[sweep_values...], Any[sweep_metrics...],
...
reverse_result = Amber.SweepResult(
    "R1.value", Any[reverse(sweep_values)...],
...
grid_result = MonteCarloResult(
    OperatingPoint(), Union{Nothing, Float64}[grid_metric...],
...
yieldmapplot(
    yield_map_figure[1, 1], grid_result; x = :x, y = :y,
    predicate = value -> value >= 0.95, bins = (18, 18)
)
save_demo("27_yield_map.png", yield_map_figure)

waterfall_figure = Figure(size = (1000, 700))
waterfall_x = collect(range(10.0, 1.0e5; length = 160))
corners = [0.7, 0.85, 1.0, 1.15, 1.3]
waterfall_values = reduce(
    hcat,
    [20log10.(1 ./ sqrt.(1 .+ (waterfall_x ./ (1.0e3 * corner)) .^ 2)) for corner in corners]
)
...
compareplot(
    comparison_figure[1, 1], [transient_result, comparison_result];
...
mos_model = ChargeBasedMOSFET(
    width = 8μm, length = 2μm,
    channel_length_modulation = 0.02
)
mos_view = mosfetview(
    mos_model; vgs = range(0.0, 1.5; length = 101),
    vds = [0.05, 0.6, 1.2]
)
...
mosfetplot(
    mos_output_figure[1, 1], mosfetview(
        mos_model;
        vgs = [0.6, 0.8, 1.0, 1.2], vds = range(0.0, 1.8; length = 101)
    ); x = :vds
)
