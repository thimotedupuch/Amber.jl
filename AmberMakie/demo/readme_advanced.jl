...
inverter_sweep = sweep(
    CMOSInverter(), "Input.dc" => range(0V, 5V; length = 201);
...
@circuit ReadmeNoiseAmplifier() begin
    ...
    Buffer = opamp(
        input, output, output, vdd, vss;
        model = BehavioralOpAmp(
            dc_gain = 1.0e5, gain_bandwidth = 1MHz,
            input_voltage_noise_density = 5.0e-9
        )
    )
    Load = resistor(output, gnd; value = 10kΩ)
end
noise_result = noise(
    ReadmeNoiseAmplifier(), 10Hz => 10MHz;
    output = voltage(:output), input = :Source, points = 220
)
noise_frequencies = frequencies(noise_result)
noise_density_nv = noise_density(noise_result) .* 1.0e9
...
for i in 2:length(noise_frequencies)
    ...
    noise_integrated_uv[i] = sqrt(noise_integrated_uv[i - 1]^2 + bin_variance * 1.0e12)
end

oscillator = transient(
    WienOscillator(), 0s => 4ms;
    saveat = 1μs, max_step = 1μs, reltol = 1.0e-6
)
...
for (column, center) in enumerate(spectrogram_times)
    spectral = column == 1 ? first_spectrum : spectrum(
            oscillator; signal = :output,
            interval = (center - 0.25ms) => (center + 0.25ms), nfft = 2048
        )
    spectrogram_db[:, column] .= 20log10.(max.(spectral.amplitude_rms[bins], 1.0e-6))
end
...
set_theme!(
    Theme(
        ...
        Axis = (
            backgroundcolor = PANEL, xgridcolor = GRID, ygridcolor = GRID,
...
Label(
    fig[1, 1:2], "AMBER  /  ADVANCED ANALYSIS";
    color = CYAN, fontsize = 22, font = :bold, halign = :left, tellwidth = false
)
Label(
    fig[2, 1:2], "Beyond the waveform";
    color = TEXT, fontsize = 47, font = :bold, halign = :left, tellwidth = false
)
Label(
    fig[3, 1:2], "Nonlinear response     ·     Correlated variation     ·     Noise integration     ·     Oscillator dynamics";
    color = MUTED, fontsize = 21, halign = :left, tellwidth = false
)

ax_inverter = Axis(
    fig[4, 1]; title = "01   CMOS  /  INVERTER NOISE MARGINS",
    xlabel = "Input voltage (V)", ylabel = "Output voltage (V)"
)
vspan!(
    ax_inverter, inverter.measurements.vil, inverter.measurements.vih;
    color = (LILAC, 0.13)
)
lines!(
    ax_inverter, [0, 5], [0, 5]; color = (MUTED, 0.48),
    linewidth = 1.5, linestyle = :dash
)
lines!(
    ax_inverter, inverter.input, inverter.output;
    color = CYAN, linewidth = 4
)
for (x, color) in (
        (inverter.measurements.vil, LILAC),
        (inverter.measurements.vih, LILAC),
    )
    vlines!(
        ax_inverter, [x]; color = (color, 0.7), linewidth = 1.6,
        linestyle = :dash
    )
end
scatter!(
    ax_inverter, [inverter.measurements.vm], [inverter.measurements.vm];
    color = CORAL, markersize = 17
)
text!(
    ax_inverter, 3.25, 4.5; text = "VM = 2.20 V", color = CORAL,
...
ax_noise = Axis(
    fig[5, 1]; title = "03   NOISE  /  SPECTRAL TO INTEGRATED",
    xlabel = "Frequency (Hz)", ylabel = "Density (nV/√Hz)", xscale = log10,
    xticks = (
        [10, 100, 1.0e3, 1.0e4, 1.0e5, 1.0e6, 1.0e7],
        ["10", "100", "1k", "10k", "100k", "1M", "10M"],
    )
)
band!(
    ax_noise, noise_frequencies, zeros(length(noise_frequencies)),
    noise_density_nv; color = (LILAC, 0.13)
)
lines!(
    ax_noise, noise_frequencies, noise_density_nv;
    color = LILAC, linewidth = 3.8
)
xlims!(ax_noise, 10, 1.0e7)
ylims!(ax_noise, 0, 6.5)
ax_integrated = Axis(
    fig[5, 1]; backgroundcolor = :transparent,
...
lines!(
    ax_integrated, noise_frequencies, noise_integrated_uv;
    color = CORAL, linewidth = 3.2
)
xlims!(ax_integrated, 10, 1.0e7)
ylims!(ax_integrated, 0, maximum(noise_integrated_uv) * 1.15)
text!(ax_noise, 30, 5.62; text = "density", color = LILAC, fontsize = 19)
text!(
    ax_integrated, 2.0e4, maximum(noise_integrated_uv) * 0.58;
    text = "integrated RMS", color = CORAL, fontsize = 19
)

ax_spectrogram = Axis(
    fig[5, 2]; title = "04   OSCILLATOR  /  STARTUP SPECTROGRAM",
    xlabel = "Time (ms)", ylabel = "Frequency (kHz)"
)
heatmap!(
    ax_spectrogram, spectrogram_times .* 1.0e3,
    spectrogram_frequencies .* 1.0e-3, permutedims(spectrogram_db);
    colormap = cgrad([PANEL, "#27617F", CYAN, GOLD, CORAL]),
    colorrange = (-85, 5)
)
hlines!(
    ax_spectrogram, [10]; color = (TEXT, 0.46),
    linewidth = 1.3, linestyle = :dash
)
text!(
    ax_spectrogram, 0.52, 13; text = "10 kHz mode", color = TEXT,
    fontsize = 18
)
...
Label(
    fig[6, 1:2], "201 bias points   •   180 correlated trials   •   220 noise frequencies   •   4,001 oscillator samples";
    color = MUTED, fontsize = 17, halign = :left, tellwidth = false
)
rowgap!(fig.layout, 15)
colgap!(fig.layout, 38)
rowsize!(fig.layout, 4, Relative(0.4))
rowsize!(fig.layout, 5, Relative(0.4))
