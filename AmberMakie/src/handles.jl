...
function _plot_observable(value, attribute)
    direct = try
        getproperty(value, attribute)
    catch
        nothing
    end
    direct === nothing || return direct
    attributes = try
        getproperty(value, :attributes)
    catch
        nothing
    end
...
helptext(handle::WorkbenchHandle) = String(
    get(
        handle.measurements, :help,
...
function _contextualize!(handle::WorkbenchHandle; help = nothing)
    ...
    legend_control = isempty(labels) ? nothing : Makie.Menu(
            toolbar[1, 3];
            options = vcat([("Trace legend · all", "")], [(label, label) for label in labels]),
            default = 1
        )
    help_label = Makie.Label(
        toolbar[1, 4], Makie.lift(help_visible) do visible
            visible ? helptext(handle) : "H: help · A: show all"
        end; halign = :left, justification = :left
    )
    subscriptions = Any[]
    push!(
        subscriptions, Makie.on(help_button.clicks) do _
            help_visible[] = !help_visible[]
        end
    )
    push!(
        subscriptions, Makie.on(show_button.clicks) do _
            showalltraces!(handle)
        end
    )
    legend_control === nothing || push!(
        subscriptions,
    ...
    push!(
        subscriptions, Makie.on(Makie.events(handle.figure).keyboardbutton) do event
