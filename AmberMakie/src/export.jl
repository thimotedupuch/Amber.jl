function _handle_figure(handle)
    ...
    return Makie.get_figure(layout isa Makie.GridLayout ? layout[1, 1] : layout)
end
    ...
    for (name, axis) in pairs(axes)
        if axis isa Makie.Axis
            ...
            output[String(name)] = Dict(
                "x" => [
                    Float64(limits.origin[1]),
                    Float64(limits.origin[1] + limits.widths[1]),
                ],
                "y" => [
                    Float64(limits.origin[2]),
...
_metadata_value(value::Pair) = Dict(
    "first" => _metadata_value(first(value)),
    "last" => _metadata_value(last(value))
)
_metadata_value(value::Tuple) = [_metadata_value(item) for item in value]
_metadata_value(value::NamedTuple) = Dict(
    String(key) => _metadata_value(item)
        for (key, item) in pairs(value)
)
_metadata_value(value::AbstractDict) = Dict(
    String(key) => _metadata_value(item)
...
function _metadata_value(value::AbstractArray)
    ...
    ndims(value) == 2 && return [
        [
            _metadata_value(value[row, column])
                for column in axes(value, 2)
        ] for row in axes(value, 1)
    ]
    return [_metadata_value(selectdim(value, 1, index)) for index in axes(value, 1)]
end
...
function copyrecipe(handle::PlotHandle)
    ...
    return "handle = $(call)$(_axis_recipe(handle.axes))$(_style_recipe(handle.plots))"
end
...
function _workbench_constructor(handle)
    ...
    return "handle = workbench(result; $(rendered))"
end
...
function copyrecipe(handle::WorkbenchHandle)
    ...
    for key in (
            :selected_order, :selected_sideband, :mode_index,
    ...
    return "$(_workbench_constructor(handle))$(selection_recipe)$(cursor)" *
...
function _report_metrics(result)
    if result isa Amber.NoiseResult
        variance = _band_variance(
            result.frequencies, result.output_psd,
            first(result.frequencies), last(result.frequencies)
        )
        return (
            frequency_points = length(result.frequencies),
    ...
    elseif result isa Amber.SimulationResult
        return (
            points = length(result.axis), axis_start = first(result.axis),
...
function _report_metric_text(metrics)
    return join(
        (
            "$(replace(titlecase(String(key)), '_' => ' ')): $(_report_value(value))"
...
function _report_metric_table(position, metrics)
    ...
    for (index, (key, value)) in enumerate(pairs(metrics))
        ...
        push!(
            labels, Makie.Label(
                grid[row, column + 1], _report_value(value);
                halign = :left, fontsize = 10, font = :bold, color = :gray20
            )
        )
    end
    return (layout = grid, labels)
end

function _report_annotation(
        template, result; input = nothing, output = nothing,
...
function _report_provenance_block(position, provenance, template, warnings)
    ...
    header = Makie.Label(
        grid[1, 1:4], "Provenance";
        halign = :left, fontsize = 10, font = :bold
    )
    entries = [
        ("Amber", version), ("Analysis", analysis),
...
function reportfigure(
        result; template = :auto, signals = nothing, signal = nothing,
    ...
    return Makie.with_theme(theme) do
        ...
        title = Makie.Label(
            figure[1, 1], "Amber · $(titlecase(String(selected))) report";
        ...
        elseif selected === :transient && result isa Amber.SimulationResult
            ...
            push!(
                handles, spectrogramplot(
                    layout[2, 1], result; signal = chosen,
        ...
        annotation_text = _report_annotation(
            selected, result; input, output,
            parameter, element
        )
        annotation = Makie.Label(
            figure[4, 1], annotation_text;
            halign = :left, fontsize = 9, color = :gray35
        )
        provenance_block = _report_provenance_block(
            figure[5, 1], provenance,
            selected, warnings
        )
        PlotHandle(
            figure, (
                report = layout, metrics = metric_table.layout,
