function _numeric_successes(view::EnsembleView)
    ...
    return indices, Float64[view.metrics[index] for index in indices]
end
...
function sweepplot(position, result::Amber.SweepResult; x = :parameter, metric = :value, failed = :mark, kwargs...)
    ...
    return PlotHandle(slot, (sweep = axis,), (curve = curve, failures = failures), view)
end

function ensembleplot(
        position, result::Amber.MonteCarloResult; metric = :value,
    ...
    if confidence && length(values) >= 2
        ...
        confidence_band = Makie.vspan!(
            axis, first(interval), last(interval);
            color = (_AMBER_COLORS.output, 0.15)
        )
        mean_marker = Makie.vlines!(
            axis, [sum(values) / length(values)];
    ...
    Makie.text!(
        axis, 0.98, 0.98; text = failure_text, space = :relative,
    ...
    if predicate !== nothing
        ...
        Makie.text!(
            axis, 0.98, 0.9; text = "Yield: $(round(100rate; digits = 1))%",
...
function outlier_samples(result::Amber.MonteCarloResult; fence = 1.5)
    ...
    return [index for index in indices if result.values[index] < lower || result.values[index] > upper]
end

function correlationplot(
        position, result::Amber.MonteCarloResult; parameter,
        metric = :value, mark_outliers = true, kwargs...
    )
    key = Symbol(parameter)
    indices = [
        index for index in eachindex(result.values) if result.converged[index] &&
...
function compareplot(
        position, results; signals, alignment = :strict, delta = :absolute,
    ...
    delta_axis = Makie.Axis(
        layout[2, 1]; xlabel = "Axis",
    ...
    for signal in signal_list
        ...
        if allowed !== nothing
            ...
            Makie.band!(
                delta_axis, reference_axis, -displayed_allowed, displayed_allowed;
        ...
        for (ri, result) in enumerate(result_list)
            ...
            push!(
                diagnostics, (
                    result = ri, signal, compared = length(valid),
            ...
            push!(
                overlay_plots, Makie.lines!(
                    axis, reference_axis, values;
                    label = "$(signal) [$(ri)]", kwargs...
                )
            )
            ri == 1 || push!(
                delta_plots, Makie.lines!(
                    delta_axis, reference_axis,
                    displayed_delta; label = "$(signal) [$(ri)]"
                )
            )
            isempty(mismatch) || Makie.scatter!(
                delta_axis, reference_axis[mismatch],
    ...
    return PlotHandle(
        layout, (comparison = axis, delta = delta_axis),
...
function yieldplot(
        position, result::Amber.MonteCarloResult; predicate = nothing,
    ...
    limits === nothing || Makie.band!(
        axis, [first(indices), last(indices)],
...
function failureplot(position, result::Union{Amber.SweepResult, Amber.MonteCarloResult}; kwargs...)
    ...
    slot = _position(position); axis = Makie.Axis(
        slot; xlabel = "Failures",
        yticks = (eachindex(labels), labels), ylabel = "Error type"
    )
    plot = Makie.barplot!(
        axis, eachindex(values), values; direction = :x,
        color = _AMBER_COLORS.invalid, kwargs...
    )
    return PlotHandle(
        slot, (failures = axis,), (failures = plot,),
...
function _pearson(x, y)
    ...
    return denominator == 0 ? NaN : sum(xcentered .* ycentered) / denominator
end
...
function rank_correlation(x::AbstractVector, y::AbstractVector)
    ...
    return _pearson(_tied_ranks(x), _tied_ranks(y))
end

function parametermatrixplot(
        position, result::Amber.MonteCarloResult;
    ...
    matrix = [
        correlate(columns[row], columns[column])
    ...
    return PlotHandle(
        slot, (parameter_matrix = axis,), (parameter_matrix = plot,),
        (
...
function _sweep_curve(result::Amber.SweepResult)
    ...
    return Float64[result.parameter_values[index] for index in selected],
...
function transfercharacteristicplot(
        position, forward::Amber.SweepResult;
    ...
    if reverse !== nothing
        ...
        all(isfinite, reverse_values) || throw(
            ArgumentError(
    ...
    derivative_axis = Makie.Axis(
        layout[2, 1]; xlabel = String(input_label),
    ...
    derivative_plot = Makie.lines!(
        derivative_axis, x, derivative;
        color = _AMBER_COLORS.input
    )
    hysteresis_plot = hysteresis === nothing ? nothing :
        Makie.band!(
            transfer_axis, x, min.(y, reverse_values), max.(y, reverse_values);
            color = (_AMBER_COLORS.warning, 0.16)
        )
    return PlotHandle(
        layout, (transfer = transfer_axis, derivative = derivative_axis),
        (
...
function sensitivityplot(
        position, sensitivities::AbstractDict; top = 20,
        metric = :value, kwargs...
    )
    ...
    return PlotHandle(
        slot, (sensitivity = axis,), (sensitivity = plot,),
        (labels = labels, values = values, metric = metric)
    )
end

function yieldmapplot(
        position, result::Amber.MonteCarloResult;
    ...
    rates = [
        totals[xi, yi] == 0 ? NaN : passed[xi, yi] / totals[xi, yi]
