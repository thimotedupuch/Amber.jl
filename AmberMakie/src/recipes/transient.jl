...
function traceplot(
        position, result::Amber.SimulationResult; signals, interval = nothing,
    ...
    plots = [
        Makie.lines!(
            ax, view.axis[selected], _transform(view.values[selected], transform);
            label = mixed_units ? "$(view.label) [$(view.unit)]" : view.label, kwargs...
        )
            for view in views
    ]
    Makie.axislegend(ax)
    return PlotHandle(slot, (trace = ax,), plots, views)
end

function eyediagramplot(
        position, result::Amber.SimulationResult; signal, period,
    ...
    slot = _position(position)
    axis = Makie.Axis(
        slot; xlabel = "Unit intervals", ylabel = view.signal,
    ...
    if !isempty(view.violations)
        violations = Makie.scatter!(
            axis, getproperty.(view.violations, :phase),
    ...
    return PlotHandle(slot, (eye = axis,), (traces = traces, violations = violations), view)
end
...
function jitterplot(
        position, result::Amber.SimulationResult; signal,
    ...
    elseif kind === :bathtub
        ...
        axis = Makie.Axis(
            slot; xlabel = "Sampling offset (s)", ylabel = "Estimated BER",
            yscale = log10
        )
