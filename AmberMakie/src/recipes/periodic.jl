...
function floquet_mode(result::Amber.PSSResult, selector = :dominant)
    ...
    labels = [
        Amber._unknown_label(result.orbit.compiled, state)
            for state in eachindex(raw)
    ]
    return FloquetModeReadout(
        index, multiplier, labels, raw ./ total,
...
function pssplot(position, result::Amber.PSSResult; signals, cycles = 1, kwargs...)
    ...
    floquet_axis = Makie.Axis(
        layout[1, 2]; xlabel = "Real", ylabel = "Imaginary",
    ...
    unit_circle = Makie.lines!(
        floquet_axis, cos.(θ), sin.(θ); color = :gray55,
        linestyle = :dash
    )
    stable = abs.(result.floquet_multipliers) .<= 1 + sqrt(eps(Float64))
    multipliers = Makie.scatter!(
        floquet_axis, real.(result.floquet_multipliers),
    ...
    Makie.xlims!(
        floquet_axis, -1.15max(1, maximum(abs, real.(result.floquet_multipliers); init = 0)),
        1.15max(1, maximum(abs, real.(result.floquet_multipliers); init = 0))
    )
    Makie.ylims!(
        floquet_axis, -1.15max(1, maximum(abs, imag.(result.floquet_multipliers); init = 0)),
...
function orbitplot(position, result::Amber.PSSResult; x, y, kwargs...)
    ...
    return PlotHandle(slot, (orbit = axis,), (orbit = plot,), result)
end
