...
function integratednoiseplot(position, result::Amber.NoiseResult; referred=:output, kwargs...)
    ...
    slot = _position(position)
    axis = _frequency_axis(slot, view.frequencies; xlabel="Frequency (Hz)",
    ...
    PlotHandle(slot, (integrated_noise=axis,), (integrated_noise=plot,),
        (frequencies=view.frequencies, rms=cumulative, referred, warnings=view.warnings,
            provenance=view.provenance))
end

function noisecontributionplot(position, result::Amber.NoiseResult; top=10,
    ...
    slot = _position(position)
    axis = Makie.Axis(slot; xlabel=quantity === :rms ? "Integrated RMS noise (V)" : "Integrated noise variance (V²)",
    ...
    PlotHandle(slot, (noise_contributions=axis,), (noise_contributions=plot,), view)
end
...
function noisebudgetplot(position,
    ...
    for index in axes(spectra, 1)
        ...
        push!(plots, Makie.band!(axis, frequencies, cumulative, upper;
...
function _integrated_phase_noise(result::Amber.PhaseNoiseResult, band)
    ...
    sqrt(max(2 * _band_variance(result.offset_frequencies,
        result.phase_noise_ratio, low, high), 0.0))
end

function _phase_contribution_view(result::Amber.PhaseNoiseResult, band;
        group=:component)
    low, high = Float64(first(band)), Float64(last(band))
    first(result.offset_frequencies) <= low < high <= last(result.offset_frequencies) ||
    ...
    for (label, spectrum) in grouped
        push!(totals, label => 2 * _band_variance(result.offset_frequencies,
            spectrum, low, high))
    end
    sort!(totals; by=last, rev=true)
    total = 2 * _band_variance(result.offset_frequencies,
        result.phase_noise_ratio, low, high)
    NoiseContributionView(string.(first.(totals)), last.(totals), total,
        low => high, :phase, _warnings(result.stats), _provenance(result))
end

function phasenoiseplot(position, result::Amber.PhaseNoiseResult;
    ...
    slot = _position(position); axis = _frequency_axis(slot, result.offset_frequencies;
        xlabel="Offset frequency (Hz)", ylabel="Phase noise (dBc/Hz)")
    plot = Makie.lines!(axis, result.offset_frequencies,
        result.phase_noise_dbc_per_hz; kwargs...)
    Makie.vspan!(axis, Float64(first(band)), Float64(last(band));
    ...
    Makie.text!(axis, 0.98, 0.98; text="$(carrier)\n$(metric)", space=:relative,
        align=(:right, :top))
    warnings = _warnings(result.stats)
    isempty(warnings) || Makie.text!(axis, 0.02, 0.02; text=join(warnings, "\n"),
        space=:relative, align=(:left, :bottom), color=_AMBER_COLORS.warning)
    PlotHandle(slot, (phase_noise=axis,), (phase_noise=plot,),
...
function periodicnoiseplot(position, result::Amber.PeriodicNoiseResult;
    ...
    axis = _frequency_axis(layout[1, 1], result.offset_frequencies;
    ...
    frequency_edges = vcat(first(frequencies) / sqrt(first(ratios)),
    ...
    PlotHandle(layout, (periodic_noise=axis,), (sidebands=plot,),
