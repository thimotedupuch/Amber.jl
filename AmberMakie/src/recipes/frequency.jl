...
function bodeplot(position, response::Union{Amber.LinearFrequencyResponse,Amber.LoopGainResult};
    ...
    plots = (magnitude=Makie.lines!(magnitude_axis, frequencies, gain; kwargs...),
        phase=Makie.lines!(phase_axis, frequencies, phase; kwargs...))
    PlotHandle(layout, (magnitude=magnitude_axis, phase=phase_axis), plots, response)
end

function bodeplot(position, result::Amber.SimulationResult; input, output, magnitude=:db,
        unwrap=true, kwargs...)
    magnitude in (:db, :linear) || throw(ArgumentError("magnitude must be :db or :linear"))
    ...
    plots = (magnitude=Makie.lines!(magnitude_axis, view.frequencies, gain; label, kwargs...),
