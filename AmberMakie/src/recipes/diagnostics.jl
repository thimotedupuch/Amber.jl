function operatingpointplot(position, result::Amber.SimulationResult;
    ...
    node_axis = Makie.Axis(layout[1, 1]; xlabel="Node", ylabel="Voltage (V)",
        xticks=(collect(eachindex(node_names)), node_names))
    node_plot = Makie.barplot!(node_axis, collect(eachindex(node_values)), node_values;
    ...
    device_axis = Makie.Axis(layout[2, 1]; xlabel="Device", ylabel="Power (W)",
        xticks=(collect(eachindex(device_names)), device_names))
    device_plot = isempty(device_values) ? nothing :
        Makie.barplot!(device_axis, collect(eachindex(device_values)), device_values;
            color=ifelse.(device_values .>= 0, _AMBER_COLORS.nominal, _AMBER_COLORS.input))
    isempty(device_values) && Makie.text!(device_axis, 0.5, 0.5;
...
function diagnosticplot(position, result; kwargs...)
    ...
    warning_label = Makie.Label(layout[1, 1], warning_text;
    ...
    device_label = Makie.Label(layout[2, 1], device_text;
        justification=:left, halign=:left, valign=:top, color=:gray35)
    PlotHandle(layout, NamedTuple(),
        (diagnostics=warning_label, device_validity=device_label), grouped)
end

function safeoperatingareaplot(position, result::Amber.SimulationResult;
    ...
    violations = findall(index -> v[index] > max_voltage || i[index] > max_current ||
...
function _average_trace(axis, values)
    ...
    Float64(real(_trapezoid(axis, values) / duration))
end

function powerdashboard(position, result::Amber.SimulationResult;
        devices=nothing, output=nothing, kwargs...)
    available = String[string(device.path) for device in
    ...
    for name in selected
        value = try _average_trace(result.axis, Amber.power(result, name)) catch; continue end
    ...
    slot = _position(position); layout = Makie.GridLayout(slot)
    axis = Makie.Axis(layout[1, 1]; xlabel="Average power (W)", ylabel="Device",
    ...
    PlotHandle(layout, (power=axis,), (power=plot,),
