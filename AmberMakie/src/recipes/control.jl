function _frequency_response(response::Amber.LinearFrequencyResponse; input = 1, output = 1)
    return response.frequencies, vec(response.values[output, input, :])
end
...
function nyquistplot(position, response; critical_point = -1 + 0im, input = 1, output = 1, kwargs...)
    ...
    return PlotHandle(slot, (nyquist = axis,), (curve = curve, critical = critical), response)
end
...
function nicholsplot(position, response; input = 1, output = 1, kwargs...)
    ...
    return PlotHandle(slot, (nichols = axis,), (nichols = plot,), response)
end
...
function polezeroplot(position, model::Amber.LinearizedModel; kwargs...)
    ...
    zeroplot = Makie.scatter!(
        axis, real.(zeros), imag.(zeros); marker = :circle, color = :transparent,
        strokecolor = _AMBER_COLORS.output, strokewidth = 2, label = "Zeros"
    )
    Makie.axislegend(axis)
    return PlotHandle(slot, (polezero = axis,), (poles = poleplot, zeros = zeroplot), model)
end
...
function rootlocusplot(position, model::Amber.LinearizedModel, gains; input = 1, output = 1, kwargs...)
    ...
    axis = Makie.Axis(slot; xlabel = "Real (rad/s)", ylabel = "Imaginary (rad/s)")
    plot = Makie.scatter!(
        axis, reduce(vcat, real.(roots)), reduce(vcat, imag.(roots));
        color = repeat(gain_values; inner = length(first(roots))), colormap = :viridis, kwargs...
    )
    return PlotHandle(slot, (rootlocus = axis,), (rootlocus = plot,), roots)
end
...
function marginplot(position, result::Amber.LoopGainResult; kwargs...)
    ...
    plots = (
        gain = Makie.lines!(gainaxis, frequencies, 20log10.(abs.(values)); kwargs...),
    ...
    return PlotHandle(layout, (magnitude = gainaxis, phase = phaseaxis), plots, result)
end
...
function groupdelayplot(position, response; input = 1, output = 1, kwargs...)
    ...
    return PlotHandle(slot, (groupdelay = axis,), (groupdelay = plot,), (frequencies, delay))
end

function poleparticipationplot(
        position, model::Amber.LinearizedModel;
    ...
    plot = Makie.barplot!(
        axis, collect(eachindex(order)), participation[order];
        direction = :x, kwargs...
    )
    return PlotHandle(
        slot, (participation = axis,), (participation = plot,),
        (
