...
"""
function mosfetview(
        model::Amber.ChargeBasedMOSFET; kind = :nmos,
        vgs = range(0.0, 1.5; length = 101), vds = [0.05, 0.6, 1.2], vbs = 0.0, temperature = 300.0
    )
    ...
    polarity = kind === :nmos ? 1.0 : -1.0
    points = [
        Amber.mosfet_operating_point(
            model, kind, polarity * d, polarity * g, 0.0, polarity * vbs;
            temperature
        ) for g in gates,d in drains
    ]
    warnings = ["Illustrative long-channel model; no foundry calibration or short-channel effects."]
    provenance = Dict(
        :model => "ChargeBasedMOSFET", :parameters => Amber.model_parameters(model),
        :kind => kind, :vgs => gates, :vds => drains, :vbs => Float64(vbs), :temperature => Float64(temperature)
    )
    return MOSFETView(model, kind, gates, drains, Float64(vbs), Float64(temperature), points, warnings, provenance)
end
...
const _MOS_LABELS = Dict(
    :id => "|ID| (A)", :gm => "|gm| (S)", :gds => "|gds| (S)",
...
function mosfetplot(position, view::MOSFETView; quantity = :id, x = :vgs, scale = :linear, axis = (;), legend = true, kwargs...)
    ...
    attributes = merge(
        (
            xlabel = x === :gm_over_id ? "gm / |ID| (1/V)" : x === :vgs ? _mos_gate_label(view) : _mos_drain_label(view) * " (V)",
            ylabel = _MOS_LABELS[quantity], yscale = scale === :log ? log10 : identity,
            xtickformat = _engineering_ticks, ytickformat = _engineering_ticks,
        ), axis
    )
    ...
    return PlotHandle(slot, (characteristic = ax,), (curves = plots,), view)
end
mosfetplot(
    position, model::Amber.ChargeBasedMOSFET; kind = :nmos, vgs = range(0.0, 1.5; length = 101),
    vds = [0.05, 0.6, 1.2], vbs = 0.0, temperature = 300.0, kwargs...
) =
...
function gmidplot(position, view::MOSFETView; axis = (;), legend = true, kwargs...)
    ...
    ax = Makie.Axis(
        slot; merge(
            (
                xlabel = "|ID| / total width (A/m)", ylabel = "gm / |ID| (1/V)",
                xscale = log10, xtickformat = _engineering_ticks,
            ), axis
        )...
    )
    ...
    return PlotHandle(slot, (gmid = ax,), (curves = plots,), view)
end
...
function capacitanceplot(position, point::NamedTuple; kwargs...)
    ...
    ax = Makie.Axis(
        layout[1, 1]; xlabel = "Voltage terminal", ylabel = "Charge terminal",
        xticks = (1:4, names), yticks = (1:4, names), yreversed = true,
        title = "Signed ∂Qi / ∂Vj"
    )
    values = Float64.(matrix) .* 1.0e15; extent = maximum(abs, values)
    extent = iszero(extent) ? 1.0 : extent
    plot = Makie.heatmap!(
        ax, 1:4, 1:4, permutedims(values); colormap = :RdBu,
        colorrange = (-extent, extent), kwargs...
    )
    Makie.Colorbar(layout[1, 2], plot; label = "fF")
    return PlotHandle(layout, (capacitance = ax,), (capacitance = plot,), point)
end
...
function workbench(view::MOSFETView)
    ...
    Makie.Legend(
        header[2, 1], transfer.plots.curves,
    ...
    subscriptions = Any[]
    push!(
        subscriptions, Makie.on(gate_slider.value) do voltage
            selected_gate[] = argmin(abs.(view.vgs .- voltage))
        end
    )
    push!(
        subscriptions, Makie.on(drain_menu.selection) do index
    ...
    Makie.colsize!(figure.layout, 1, Makie.Relative(0.5))
    Makie.colsize!(figure.layout, 2, Makie.Relative(0.5))
    plots = (
        transfer = transfer.plots.curves, gmid = efficiency.plots.curves,
        gain = gain.plots.curves, capacitance = capacitance.plots.curves, selected = markers,
    )
    measurements = Dict{Symbol, Any}(
        :selected_gate => selected_gate, :selected_drain => selected_drain,
    ...
    handle = WorkbenchHandle(
        figure, axes, plots, Makie.Observable(Symbol[]), nothing, measurements,
        copy(view.warnings), copy(view.provenance), cleanup, false
    )
    return _contextualize!(handle; help = "Choose a drain bias and move the gate slider; click a voltage-domain plot to inspect the nearest stored bias point. Units and model parameters are retained on export.")
end
