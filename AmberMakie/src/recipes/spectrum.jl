function spectrumplot(position, result::Amber.SpectrumResult; scale=:rms,
    ...
    axis = frequency_scale === :log ? _frequency_axis(slot, view.frequencies[indices];
        xlabel="Frequency (Hz)", ylabel) : Makie.Axis(slot; xlabel="Frequency (Hz)", ylabel)
    ...
    if fundamental !== nothing
        classifications = [spectrum_cursor(result, frequency; fundamental).classification
        ...
        isempty(harmonic_indices) || Makie.scatter!(axis,
            view.frequencies[indices][harmonic_indices], values[indices][harmonic_indices];
            color=ifelse.(classifications[harmonic_indices] .=== :fundamental,
                _AMBER_COLORS.output, _AMBER_COLORS.candidate), markersize=8)
    end
    PlotHandle(slot, (spectrum=axis,), (spectrum=plot, band=band_annotation), view)
end
...
function spectrogramplot(position, result::Amber.SimulationResult; signal, window=:hann,
...
function waterfallplot(position, x::AbstractVector, sweeps::AbstractVector,
    ...
    size(values) == (length(x), length(sweeps)) || throw(DimensionMismatch(
    ...
    plots = [Makie.lines!(axis, Float64.(x), fill(Float64(sweeps[index]), length(x)),
        displayed[:, index]; color=index, colormap=:viridis, colorrange=(1, length(sweeps)),
        kwargs...) for index in eachindex(sweeps)]
    PlotHandle(slot, (waterfall=axis,), (waterfall=plots,),
