...
function engineering(value::Real; unit = "", digits::Integer = 4)
    ...
    return string(rendered, " ", prefix, unit)
end
...
function _log_minor_ticks(values)
    ...
    return [multiplier * 10.0^decade for decade in first_decade:last_decade for multiplier in 2:9]
end
...
function _log_major_ticks(values)
    ...
    return ticks, [strip(engineering(value; unit = "Hz", digits = 1)) for value in ticks]
end

function _frequency_axis(position, frequencies; kwargs...)
    return Makie.Axis(
        position; xscale = log10, xticks = _log_major_ticks(frequencies),
