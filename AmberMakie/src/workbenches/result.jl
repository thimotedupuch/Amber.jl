function _workbench_figure()
    return Makie.Figure(size = (1000, 700))
end
...
function _workbench_handle(figure, handle)
    ...
    primary === nothing && return WorkbenchHandle(
        figure, handle.axes, handle.plots,
    ...
    push!(
        cursors.subscriptions, Makie.on(interval_slider.interval) do bounds
    ...
    return _contextualize!(
        WorkbenchHandle(
            figure, handle.axes, handle.plots, selection,
...
function _operatingpoint_workbench(result::Amber.SimulationResult)
    ...
    append!(
        options, [
            ("$(uppercase(String(row.kind))) · $(row.name)", row.name)
    ...
    push!(
        subscriptions, Makie.on(menu.selection) do value
    ...
    search_results = Makie.lift(search.stored_string) do query
        ...
        names = vcat(
            ["V($(row.name))" for row in filtered.nodes],
            ["$(row.kind) · $(row.name)" for row in filtered.devices]
        )
        isempty(names) ? "No matches" : join(first(names, min(12, length(names))), "\n")
    end
    Makie.Label(
        figure[4, 2], search_results; halign = :left, valign = :top,
        justification = :left
    )

    push!(
        subscriptions, Makie.on(selected) do name
    ...
    node_highlight = Makie.scatter!(
        plot_handle.axes.nodes, node_point;
        color = _AMBER_COLORS.warning, marker = :diamond, markersize = 16
    )
    device_highlight = Makie.scatter!(
        plot_handle.axes.devices, device_point;
        color = _AMBER_COLORS.warning, marker = :diamond, markersize = 16
    )
    ...
    Makie.Label(
        figure[6, 2], convergence_text; halign = :left, justification = :left,
        color = convergence.converged ? :gray35 : _AMBER_COLORS.invalid
    )
    convergence_axis = Makie.Axis(
        figure[6:7, 1]; xlabel = "Newton iteration",
    ...
    if convergence.history_available && !isempty(convergence.history)
        ...
        push!(
            convergence_plots, Makie.lines!(
                convergence_axis, iterations, residuals;
                label = "Residual ‖r‖∞", color = _AMBER_COLORS.invalid
            )
        )
        push!(
            convergence_plots, Makie.lines!(
                convergence_axis, iterations, updates;
    ...
    measurements = Dict{Symbol, Any}(
        :selected => selected, :browser => menu,
    ...
    return _contextualize!(
        WorkbenchHandle(
            figure, axes, plots, selection, nothing,
...
function _smallsignal_workbench(
        result::Amber.SimulationResult; input, output,
    ...
    magnitude_axis = _frequency_axis(
        figure[1:2, 1], frequencies;
        ylabel = "Gain (dB)"
    )
    phase_axis = _frequency_axis(
        figure[3:4, 1], frequencies;
        xlabel = "Frequency (Hz)", ylabel = "Phase (°)"
    )
    Makie.linkxaxes!(magnitude_axis, phase_axis)
    nyquist_axis = Makie.Axis(
        figure[1:4, 2]; xlabel = "Real", ylabel = "Imaginary",
        autolimitaspect = 1
    )
    ...
    marker_a = Makie.scatter!(
        nyquist_axis, point(index_a); color = _AMBER_COLORS.input,
        marker = :circle, markersize = 14
    )
    marker_b = Makie.scatter!(
        nyquist_axis, point(index_b); color = _AMBER_COLORS.output,
        marker = :diamond, markersize = 14
    )

    Makie.Label(figure[5, 1], "Input")
    input_menu = Makie.Menu(
        figure[6, 1];
        options = [(_signal_label(choice), choice) for choice in input_choices], default = 1
    )
    Makie.Label(figure[5, 2], "Output")
    output_menu = Makie.Menu(
        figure[6, 2];
        options = [(_signal_label(choice), choice) for choice in output_choices], default = 1
    )
    subscriptions = Any[]
    push!(
        subscriptions, Makie.on(input_menu.selection) do choice
            choice === nothing || (selected_input[] = choice)
        end
    )
    push!(
        subscriptions, Makie.on(output_menu.selection) do choice
            choice === nothing || (selected_output[] = choice)
        end
    )
    append!(
        subscriptions, Makie.onany(selected_input, selected_output) do source, sink
            view = frequencyview(result; input = source, output = sink)
            response[] = ComplexF64.(view.response)
            copyto!(cursor_gain, gain[])
            cursors.readout[] = cursor_readout(
                frequencies, cursor_gain,
                cursors.a[], cursors.b[]
            )
        end
    )
    push!(
        subscriptions, Makie.on(
            Makie.events(nyquist_axis).mousebutton;
            priority = 10
        ) do event
    ...
    push!(
        subscriptions, Makie.on(Makie.events(figure).keyboardbutton) do event
    ...
    readout = Makie.lift(cursors.a, cursors.b, response) do a_frequency, b_frequency, values
        ...
        (
            a = (
                frequency = a.x, response = a.y, gain_db = 20log10(abs(a.y)),
                phase_degrees = rad2deg(angle(a.y)),
            ),
            b = (
                frequency = b.x, response = b.y, gain_db = 20log10(abs(b.y)),
    ...
    measurements = Dict{Symbol, Any}(
        ...
        :copyrecipe_kwargs => Dict(
            :input => repr(input), :output => repr(output),
    ...
    plots = (
        magnitude = magnitude_plot, phase = phase_plot, nyquist = nyquist_plot,
    ...
    return _contextualize!(
        WorkbenchHandle(
            figure, axes, plots, selection, cursors,
...
function workbench(
        result::Amber.SimulationResult; signals = nothing, input = nothing,
    ...
    if result.analysis isa Union{Amber.Transient, Amber.TransientNoise}
        ...
        signal_control = Makie.Menu(
            sidebar[7, 1];
            options = [(_signal_label(signal), signal) for signal in signal_choices], default = 1
        )
        push!(
            workbench_handle.measurements[:control_subscriptions],
        ...
        push!(
            ...
            Makie.on(selected_signal) do signal
                ...
                if !isempty(finite_values)
                    ...
                    padding = high == low ? (iszero(low) ? 1.0 : 0.05abs(low)) : 0.05(high - low)
                ...
                cursors.interval_readout[] = interval_readout(
                    active.axis,
    ...
    return _contextualize!(workbench_handle)
end
...
function workbench(result::Amber.SpectrumResult; fundamental = nothing, view = :auto)
    ...
    band_power = Makie.Observable(
        Amber.band_power(
            result,
            first(frequencies) => last(frequencies)
        )
    )
    slider = Makie.IntervalSlider(
        figure[2, 2]; range = frequencies,
        startvalues = (first(frequencies), last(frequencies))
    )
    push!(
        cursors.subscriptions, Makie.on(slider.interval) do bounds
            setinterval!(cursors, Float64(first(bounds)) => Float64(last(bounds)))
        end
    )
    push!(
        cursors.subscriptions, Makie.on(cursors.interval) do band
    ...
    Makie.Label(
        figure[3:5, 2], annotation; halign = :left, valign = :top,
        justification = :left
    )
    inspector = Makie.DataInspector(figure)
    cleanup = () -> begin
...
function workbench(result::Amber.HarmonicResult; view = :auto)
    ...
    subscriptions = Any[]
    push!(
        subscriptions, Makie.on(slider.value) do order
            selected_order[] = Int(order)
        end
    )
    push!(
        subscriptions, Makie.on(selected_order) do order
    ...
    Makie.Label(
        figure[4:5, 2], metrics; halign = :left, valign = :top,
        justification = :left
    )
    inspector = Makie.DataInspector(figure)
    cleanup = () -> begin
...
function workbench(result::Amber.NoiseResult; referred = :output, view = :auto)
    ...
    contribution_axis = Makie.Axis(
        figure[1:4, 2]; xlabel = "Integrated RMS noise (V)",
        ylabel = "Component"
    )
    contribution_plot = Makie.barplot!(
        contribution_axis, contribution_values;
        direction = :x
    )
    ...
    slider = Makie.IntervalSlider(
        figure[5, 2]; range = frequencies,
        startvalues = (first(frequencies), last(frequencies))
    )
    band_rms = Makie.Observable(
        sqrt(
            max(
                _band_variance(
                    frequencies, psd,
    ...
    return _contextualize!(
        WorkbenchHandle(
            figure, axes, plots, Makie.Observable(Symbol[]),
...
function workbench(
        result::Amber.PhaseNoiseResult; carrier_frequency = nothing,
    ...
    contribution_axis.yticks = (
        collect(eachindex(initial_contributions.labels)),
    ...
    slider = Makie.IntervalSlider(
        figure[5, 2]; range = result.offset_frequencies,
    ...
    push!(
        cursors.subscriptions, Makie.on(cursors.interval) do selected
            ...
            contribution_axis.yticks[] = (
                collect(eachindex(contributions.labels)),
    ...
    Makie.Label(
        figure[7, 1:2], validity_text; halign = :left, justification = :left,
    ...
    measurements = Dict{Symbol, Any}(
        ...
        :validity => validity,
        :copyrecipe_kwargs => Dict(
            :carrier_frequency => repr(carrier_frequency),
            :band => repr(band)
        )
    )
    axes = (
        phase_noise = handle.axes.phase_noise,
        noise_budget = budget.axes.noise_budget,
        phase_contributions = contribution_axis,
    )
    plots = (
        phase_noise = handle.plots.phase_noise,
...
function workbench(
        result::Amber.PeriodicNoiseResult; view = :auto, scale = :db,
    ...
    handle = periodicnoiseplot(
        pss === nothing ? figure[1:5, 1] : figure[1:3, 1],
        result; scale
    )
    orbit_handle = pss === nothing ? nothing : traceplot(
            figure[4:6, 1], pss.orbit;
            signals
        )
    ...
    selected_sideband = Makie.Observable(
        result.sidebands[
            argmin(abs.(result.sidebands .- result.output_harmonic)),
        ]
    )
    selection = Makie.Observable([Symbol("sideband_$(selected_sideband[])")])
    sideband_control = Makie.Slider(
        figure[2, 2]; range = result.sidebands,
        startvalue = selected_sideband[], snap = true
    )
    subscriptions = Any[]
    push!(
        subscriptions, Makie.on(sideband_control.value) do sideband
            selected_sideband[] = Int(sideband)
        end
    )
    push!(
        subscriptions, Makie.on(selected_sideband) do sideband
            selection[] = [Symbol("sideband_$(sideband)")]
        end
    )
    sideband_line = Makie.hlines!(
        handle.axes.periodic_noise, selected_sideband;
    ...
    axes = (
        periodic_noise = handle.axes.periodic_noise,
        orbit = orbit_handle === nothing ? nothing : orbit_handle.axes.trace,
    )
    plots = (
        sidebands = handle.plots.sidebands, selected_sideband = sideband_line,
        orbit = orbit_handle === nothing ? nothing : orbit_handle.plots,
    )
    return _contextualize!(
        WorkbenchHandle(
            figure, axes, plots, selection,
            cursors, measurements, _warnings(result.stats), _provenance(result), cleanup,
            false
        )
    )
...
function workbench(result::Amber.NetworkResult; parameter = :s, element = (1, 1), view = :auto)
    ...
    smith_axis = Makie.Axis(
        figure[1:2, 2]; title = smith_title,
    ...
    impedance_axis = _frequency_axis(
        figure[3, 2], frequencies;
        ylabel = "Real Z (Ω)"
    )
    reactance_axis = _frequency_axis(
    ...
    impedance_plot = Makie.lines!(
        impedance_axis, frequencies,
        Makie.lift(values -> real.(values), impedance_values)
    )
    reactance_plot = Makie.lines!(
        reactance_axis, frequencies,
    ...
    cursors = CursorState(frequencies, cursor_gain)
    foreach(
        axis -> _cursor_axis!(axis, cursors, frequencies),
    ...
    labels = [
        something(port.name, Symbol("port$(index)")) |> string
    ...
    element_options = [
        (
            "$(labels[candidate_row]) ← $(labels[candidate_column]) · [$candidate_row,$candidate_column]",
            (candidate_row, candidate_column),
        )
            for (candidate_row, candidate_column) in elements
    ]
    Makie.Label(figure[5, 1], "Network parameter"; halign = :left)
    parameter_control = Makie.Menu(
        figure[6, 1];
        options = [("S", :s), ("Z", :z), ("Y", :y)],
        default = findfirst(==(parameter), (:s, :z, :y))
    )
    Makie.Label(figure[5, 2], "Port / matrix element"; halign = :left)
    element_control = Makie.Menu(
        figure[6, 2]; options = element_options,
        default = findfirst(==(element), elements)
    )
    slider = Makie.IntervalSlider(
        figure[7, 1:2]; range = frequencies,
        startvalues = (first(frequencies), last(frequencies))
    )
    subscriptions = Any[]
    push!(
        subscriptions, Makie.on(parameter_control.selection) do value
            value === nothing || (selected_parameter[] = Symbol(value))
        end
    )
    push!(
        subscriptions, Makie.on(element_control.selection) do value
            value === nothing || (selected_element[] = Tuple(Int.(value)))
        end
    )
    append!(
        subscriptions, Makie.onany(selected_parameter, selected_element) do _, _
            copyto!(cursor_gain, gain[])
            cursors.readout[] = cursor_readout(
                frequencies, cursor_gain,
                cursors.a[], cursors.b[]
    ...
    push!(
        subscriptions, Makie.on(
            Makie.events(smith_axis).mousebutton;
            priority = 10
        ) do event
            event.button == Makie.Mouse.left && event.action == Makie.Mouse.press ||
                return Makie.Consume(false)
    ...
    inspector = Makie.DataInspector(figure)
    axes = (
        magnitude = magnitude_axis, phase = phase_axis,
        smith = smith_axis, impedance = impedance_axis, reactance = reactance_axis,
    )
    plots = (
        magnitude = magnitude_plot, phase = phase_plot,
...
function workbench(result::Amber.PSSResult; signals, view = :auto)
    ...
    mode_axis = Makie.Axis(
        figure[1:3, 2]; xlabel = "Mode participation", ylabel = "State",
    ...
    participation_marker = Makie.scatter!(
        mode_axis,
    ...
    state_axis = Makie.Axis(
        figure[5:6, 1]; xlabel = "Time (s)",
        ylabel = "Raw state", title = selected_state_label
    )
    state_plot = Makie.lines!(
        state_axis, result.orbit.axis, state_trace;
    ...
    mode_marker = Makie.scatter!(
        plot_handle.axes.floquet, floquet_point;
        color = _AMBER_COLORS.warning, marker = :diamond, markersize = 14
    )
    selector = Makie.Slider(
        figure[4, 2]; range = eachindex(modes),
    ...
    subscriptions = Any[]
    push!(
        subscriptions, Makie.on(selector.value) do index
            selected_index[] = Int(index)
        end
    )
    push!(
        subscriptions, Makie.on(signal_control.selection) do signal
            signal === nothing || (selected_signal[] = signal)
        end
    )
    push!(
        subscriptions, Makie.on(selected_signal) do signal
            index = findfirst(choice -> isequal(choice, signal), signal_choices)
            index === nothing && return
    ...
    push!(
        subscriptions, Makie.on(selected_mode) do mode
            selected_state[] = argmax(mode.participation)
        end
    )
    push!(
        subscriptions, Makie.on(
            Makie.events(plot_handle.axes.floquet).mousebutton;
            priority = 9
        ) do event
            event.button == Makie.Mouse.left && event.action == Makie.Mouse.press ||
                return Makie.Consume(false)
            ...
            selected_index[] = argmin(
                abs2.(real.(multipliers) .- target[1]) .+
    ...
    push!(
        subscriptions, Makie.on(
            Makie.events(mode_axis).mousebutton;
            priority = 9
        ) do event
            event.button == Makie.Mouse.left && event.action == Makie.Mouse.press ||
                return Makie.Consume(false)
            ...
            state = clamp(
                round(Int, Makie.mouseposition(mode_axis)[2]),
    ...
    state_text = Makie.lift(
        selected_mode, selected_state_label, selected_state,
        participation
    ) do mode, label, state, values
    ...
    push!(
        subscriptions, Makie.on(state_trace) do values
            copyto!(first_trace, values)
            cursors.readout[] = cursor_readout(
                result.orbit.axis, first_trace,
                cursors.a[], cursors.b[]
            )
        end
    )
    ...
    axes = merge(
        plot_handle.axes, (
            mode_participation = mode_axis,
            linked_state = state_axis,
        )
    )
    plots = merge(
        plot_handle.plots, (
            selected_mode = mode_marker,
    ...
    return _contextualize!(
        WorkbenchHandle(
            figure, axes, plots, selection, cursors, measurements,
...
function workbench(result::Amber.SweepResult; view = :auto)
    ...
    marker = Makie.scatter!(
        sweep_handle.axes.sweep, selected_point;
        color = _AMBER_COLORS.warning, marker = :diamond, markersize = 14
    )
    slider = Makie.Slider(
        figure[5, 1:2]; range = eachindex(result.metrics),
        startvalue = 1, snap = true
    )
    subscriptions = Any[]
    push!(
        subscriptions, Makie.on(slider.value) do sample
            selected_sample[] = Int(sample)
        end
    )
    push!(
        subscriptions, Makie.on(selected_sample) do sample
    ...
    push!(
        subscriptions, Makie.on(
            Makie.events(sweep_handle.axes.sweep).mousebutton;
            priority = 9
        ) do event
            event.button == Makie.Mouse.left && event.action == Makie.Mouse.press ||
                return Makie.Consume(false)
    ...
    Makie.Label(
        figure[4, 2], details; halign = :left, justification = :left,
        valign = :top
    )
    numeric_metrics = Float64[
        result.metrics[index] for index in eachindex(result.metrics)
    ...
    plots = (
        sweep = sweep_handle.plots.curve, failures = failure_handle.plots.failures,
        selected = marker,
    )
    measurements = Dict{Symbol, Any}(
...
function workbench(
        result::Amber.MonteCarloResult; predicate = nothing, circuit = nothing,
    ...
    axes = Dict{Symbol, Any}(
        :ensemble => ensemble.axes.ensemble,
        :failures => failures.axes.failures
    )
    plots = Dict{Symbol, Any}(
        :ensemble => ensemble.plots.histogram,
    ...
    ensemble_marker = Makie.vlines!(
        ensemble.axes.ensemble, selected_metric;
        color = _AMBER_COLORS.warning, linewidth = 3
    )
    plots[:selected_metric] = ensemble_marker

    available = sort!(
        collect(union((Set(keys(draw)) for draw in result.parameters)...));
        by = string
    )
    complete = count(
        index -> result.converged[index] && result.values[index] isa Real &&
    ...
    if !isempty(available) && complete >= 2
        ...
        correlation_marker = Makie.scatter!(
            correlation.axes.correlation,
            Makie.lift(selected_sample) do sample
                ...
                position === nothing ? [Makie.Point2f(NaN, NaN)] :
                    [
                        Makie.Point2f(
                            correlation.view.parameter[position],
    ...
    Makie.Label(
        figure[4, 2:3], sample_summary; halign = :left, valign = :top,
        justification = :left
    )
    sample_slider = Makie.Slider(
        figure[5, 1:3]; range = 1:length(result.values),
        startvalue = 1, snap = true
    )
    subscriptions = Any[]
    push!(
        subscriptions, Makie.on(sample_slider.value) do sample
            selected_sample[] = Int(sample)
        end
    )
    push!(
    ...
    push!(
        subscriptions, Makie.on(
            Makie.events(ensemble.axes.ensemble).mousebutton;
            priority = 9
        ) do event
            event.button == Makie.Mouse.left && event.action == Makie.Mouse.press ||
                return Makie.Consume(false)
            ...
            candidates = [
                index for index in eachindex(result.values)
                    if result.converged[index] && result.values[index] isa Real
            ]
            isempty(candidates) || (
                selected_sample[] = candidates[
                    argmin(
                        abs.(
    ...
    if correlation !== nothing
        push!(
            subscriptions, Makie.on(
                Makie.events(correlation.axes.correlation).mousebutton;
                priority = 9
            ) do event
                event.button == Makie.Mouse.left && event.action == Makie.Mouse.press ||
                    return Makie.Consume(false)
                ...
                scale_x = max(
                    maximum(correlation.view.parameter) -
                        minimum(correlation.view.parameter), eps(Float64)
                )
                scale_y = max(
                    maximum(correlation.view.metric) -
                        minimum(correlation.view.metric), eps(Float64)
                )
                position = argmin(
                    abs2.((correlation.view.parameter .- target[1]) ./ scale_x) .+
    ...
    push!(
        subscriptions, Makie.on(
            Makie.events(failures.axes.failures).mousebutton;
            priority = 9
        ) do event
            event.button == Makie.Mouse.left && event.action == Makie.Mouse.press ||
                return Makie.Consume(false)
            ...
            if 1 <= row <= length(failures.view.labels) && !isempty(result.failures)
                ...
                failure = findfirst(
                    item -> string(item.exception_type) == kind,
                    result.failures
                )
    ...
    numeric = Float64[
        result.values[index] for index in eachindex(result.values)
            if result.converged[index] && result.values[index] isa Real
    ]
    ...
    Makie.Label(
        figure[7, 1:3], replay_text; halign = :left, justification = :left,
