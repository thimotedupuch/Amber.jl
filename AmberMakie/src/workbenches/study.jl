...
function _parameter_initial(spec)
    ...
    return sqrt(float(spec[1]) * float(spec[2]))
end
...
function explore(runner::Function; parameters = NamedTuple(), autorun = false)
    ...
    handle = StudyHandle(
        figure, values, result, status, Dict{Any, Any}(), nothing, 0,
    ...
    for (name, spec) in pairs(parameters)
        ...
        push!(
            handle.subscriptions, Makie.on(slider.value) do value
    ...
    push!(
        handle.subscriptions, Makie.on(button.clicks) do _
            runstudy!(handle)
        end
    )
...
function explore(builder, analysis; parameters = NamedTuple(), outputs = Any[], autorun = false)
    ...
    return explore(runner; parameters, autorun)
end
...
function pin!(handle::StudyHandle)
    ...
    push!(handle.pinned, handle.result[])
    return handle
end
