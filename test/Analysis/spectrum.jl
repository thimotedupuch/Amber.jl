@testset "spectrum and harmonic analysis" begin
    ...
    @test spectral.sample_rate≈100kHz rtol=1e-10
    @test spectral.frequencies[argmax(spectral.amplitude_rms)]≈1kHz rtol=.02
    @test maximum(spectral.amplitude_rms)≈sqrt(2) rtol=.03
    @test crest_factor(spectral)≈sqrt(2) rtol=.03
    ...
    @test harmonic.fundamental.frequency≈1kHz rtol=.02
    @test thd(harmonic)<.02
    @test thdn(harmonic)<.03
end
