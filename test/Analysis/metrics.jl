...
@testset "directional delay and CMOS measurements without plotting" begin
    ...
    t=collect(0.:.025:10.)
    phase=mod.(t,5.)
    vin=[q<2 ? clamp((q-.9)/.2,0,1) : clamp((3.1-q)/.2,0,1) for q in phase]
    vout=[q<2 ? 1-clamp((q-1.1)/.2,0,1) : clamp((q-3.3)/.2,0,1) for q in phase]
    @test first(vin)==last(vin)==0
    @test_throws ArgumentError propagation_delay(t,vin,vout)
    for occurrence in (1,2)
        @test propagation_delay(t,vin,vout;threshold=.5,input_edge=:rising,
            output_edge=:falling,occurrence)≈.2 atol=1e-12
        @test propagation_delay(t,vin,vout;threshold=.5,input_edge=:falling,
            output_edge=:rising,occurrence)≈.4 atol=1e-12
    end
    @test propagation_delay(t,vin,vout;window=.5=>2.)≈.2 atol=1e-12
    @test propagation_delay(t,vin,2 .* vout;input_threshold=.5,output_threshold=1.,
        input_edge=:rising,output_edge=:falling)≈.2 atol=1e-12
    ...
    switching_measure=switchingmetrics(t,vin,vout,2 .* t .+ 1;vdd=1.,window=.25=>9.75)
    @test switching_measure.tphl≈.2 atol=1e-12
    @test switching_measure.tplh≈.4 atol=1e-12
    @test length(switching_measure.phl)==length(switching_measure.plh)==2
    @test switching_measure.energy≈104.5
    @test isnan(switchingmetrics(t,vin,ring,ones(length(t));vdd=1.,window=.25=>4.75).tphl)
    @test_throws ArgumentError switchingmetrics(t,vin,vout,ones(length(t));vdd=0.,window=0=>4)
    ...
    @test_throws ArgumentError invertermetrics([0.,0.],[1.,0.])
    ...
    @test_throws ArgumentError switchingmetrics(partial;input=:vin,output=:vout,supply=:V1,vdd=1.,window=0=>1ms)
