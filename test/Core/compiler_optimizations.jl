...
@testset "residual evaluation and conservative MOS derivatives" begin
    ...
    gradient = (v, t) -> begin
        gradient_calls[] += 1; (2v[1], 0.0, 0.0, 0.0)
    end
    @circuit EvaluationCircuit() begin
        gnd = ground(); gate = node(); drain = node(); output = node()
        VG = voltage_source(gate, gnd; dc = 1.0, ac = 1.0)
        R = resistor(drain, gnd; value = 1.0e3)
        C = capacitor(drain, gnd; value = 1.0e-9)
        B = behavioral_current_source(
            ((gate, gnd),), drain, gnd;
            current = (v, t) -> v[1]^2, gradient
        )
        BV = behavioral_voltage_source(
            ((gate, gnd),), output, gnd;
            voltage = (v, t) -> v[1]^2, gradient
        )
        M = nmos(
            drain, gate, gnd, gnd; model = ChargeBasedMOSFET(
                drain_area = 1.0e-12, source_area = 1.0e-12, gate_drain_overlap = 1.0e-10
            )
        )
    end
    cc = compile(EvaluationCircuit()); ws = SimulationWorkspace(cc)
    x = collect(range(0.1, 0.8; length = cc.n)); previous = x .- 0.01
    directions = Matrix{Float64}(I, cc.n, cc.n)
    for temperature in (280.0, 340.0), alpha in (0.0, 1.0e6)
        qhistory = Amber._storage(cc, previous; temperature)
        r, j = Amber._step_residual_jacobian!(ws, cc, x, qhistory, 0.2, alpha; temperature, gmin = 1.0e-8)
        reference = copy(r); matrix = copy(j.nzval); count = gradient_calls[]
        trial = copy(Amber._step_residual!(ws, cc, x, qhistory, 0.2, alpha; temperature, gmin = 1.0e-8))
        @test trial ≈ reference rtol = 1.0e-13
        ...
        numerical = hcat(
            [
                (
                    copy(Amber._step_residual!(ws, cc, x + h * directions[:, k], qhistory, 0.2, alpha; temperature, gmin = 1.0e-8)) -
                        copy(Amber._step_residual!(ws, cc, x - h * directions[:, k], qhistory, 0.2, alpha; temperature, gmin = 1.0e-8))
                ) / (2h)
                    for k in 1:cc.n
            ]...
        )
        @test Matrix(j) ≈ numerical rtol = 1.0e-7 atol = 1.0e-10
    end
    dx = fill(0.02, cc.n)
    r, j = residual_jacobian!(ws, cc, x, x - dx, 0.2, 1.0)
    reference = copy(r); matrix = copy(j.nzval); count = gradient_calls[]
    @test residual!(ws, cc, x, dx, 0.2) ≈ reference
    @test ws.jacobian.nzval == matrix
    @test gradient_calls[] == count
end
...
@testset "constant contribution and forcing invalidation" begin
    ...
    x = zeros(cc.n); alpha = 1.0e5
    Amber._newton(cc, x, x, 1.0e-6, alpha; workspace = ws)
    factors = ws.numeric_factorizations; assemblies = ws.constant_assemblies
    source = with_parameters(cc, "V1.dc" => 2.0)
    @test source.parameters.matrix_fingerprint == cc.parameters.matrix_fingerprint
    answer, _, converged = Amber._newton(source, x, x, 1.0e-6, alpha; workspace = ws, mode = :dc)
    @test converged
    # mode is part of the factorization key; repeat using the same mode.
    factors = ws.numeric_factorizations
    source2 = with_parameters(source, "V1.dc" => 3.0)
    answer2, _, converged = Amber._newton(source2, x, x, 1.0e-6, alpha; workspace = ws, mode = :dc)
    @test converged
    @test answer2 ≈ 1.5answer
    @test ws.numeric_factorizations == factors
    @test ws.constant_assemblies == assemblies
    changed = with_parameters(source2, "R1.value" => 2.0e4, "C1.value" => 2.0e-8)
    @test changed.parameters.matrix_fingerprint != source2.parameters.matrix_fingerprint
    answer3, _, converged = Amber._newton(changed, x, x, 1.0e-6, alpha; workspace = ws, mode = :dc)
    @test converged
    @test ws.numeric_factorizations > factors
    @test ws.constant_assemblies == assemblies + 1
    fresh, _, _ = Amber._newton(changed, x, x, 1.0e-6, alpha; mode = :dc)
    @test answer3 ≈ fresh

    cc = compile(BiasedNPN()); ws = SimulationWorkspace(cc)
    x = operating_point(cc).values[:, 1]; q = Amber._storage(cc, x)
    r, j = Amber._step_residual_jacobian!(ws, cc, x, q, 0.0, 1.0e5)
    assemblies = ws.constant_assemblies
    # Cached passive contributions coexist with temperature-dependent devices.
    for temperature in (290.0, 330.0)
        r, j = Amber._step_residual_jacobian!(ws, cc, x, q, 0.0, 1.0e5; temperature)
        expected, g = Amber.residual_jacobian(cc, x, x, 0.0, 0.0; temperature)
        storage, c = storage_jacobian!(SimulationWorkspace(cc), cc, x; temperature)
        @test r ≈ expected + 1.0e5 * (storage - q)
        @test Matrix(j) ≈ Matrix(g + 1.0e5 * c)
    end
    @test ws.constant_assemblies == assemblies
    tuned = with_parameters(cc, "Q1.forward_beta" => 90.0)
    @test tuned.parameters.matrix_fingerprint == cc.parameters.matrix_fingerprint
    r, j = Amber._step_residual_jacobian!(ws, tuned, x, q, 0.0, 1.0e5)
    expected, g = Amber.residual_jacobian(tuned, x, x, 0.0, 0.0)
...
@testset "AC symbolic reuse" begin
    cc = compile(LowPass()); fs = [0.0, 1.0e2, 1.0e4, 1.0e6]
    ...
    @test voltage(ac, :vout) ≈ 1 ./ (1 .+ im * 2π .* fs .* 1.0e4 .* 1.0e-8)
end

@testset "resolved parameter updates" begin
    cc = compile(compiler_ladder())
    handle = parameter_handle(cc, "stage[1:3].R1.value")
    changed = with_parameters(cc, handle => 2.0e3, "stage[2].R1.value" => 3.0e3, handle => 4.0e3)
    ...
    @test resistors.conductance[1:3] == fill(1 / 4.0e3, 3)
    @test original.conductance[1:3] == 1 ./ [1.0e3, 2.0e3, 3.0e3]
    @test resistors.p === original.p
    @test with_parameters(changed, handle => 5.0e3).topology === cc.topology
    @test_throws ArgumentError with_parameters(compile(compiler_ladder()), handle => 2.0e3)
    ...
    @test_throws KeyError with_parameters(cc, handle => 9.0e3, "missing.value" => 1.0)
    @test original.conductance[1:3] == 1 ./ [1.0e3, 2.0e3, 3.0e3]
    direct = with_parameters(cc, handle => 4.0e3)
...
@testset "equation structural analysis" begin
    @circuit StructuralRC() begin
        gnd = ground(); a = node()
        G = conductance(a, gnd; value = 0.0)
        C = capacitor(a, gnd; value = 1.0e-9)
    end
    ...
    @circuit FeedForward() begin
        ...
        V = voltage_source(input, gnd; dc = 1.0)
        E = voltage_controlled_voltage_source(input, gnd, output, gnd; gain = 2.0)
        R = resistor(output, gnd; value = 1.0e3)
    end
    ...
    for (block, rows) in enumerate(analysis.blocks), row in rows
        block_of[row] = block
    end
