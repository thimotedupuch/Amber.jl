...
@testset "noise PSD identities and contributions" begin
    ...
    result=noise(NoiseIdentity(),[10Hz,1kHz,10kHz];
    ...
    conductance_result=noise(ConductanceIdentity(),[1kHz];
        output=voltage(:output),temperature)
    @test noise_psd(conductance_result)≈noise_psd(noise(NoiseIdentity(),[1kHz];
        output=voltage(:output),temperature))
    ...
    rc_noise=noise(RCNoiseIdentity(),1e-3Hz=>1e12Hz;
        output=voltage(:output),temperature,points=301)
    rc_variance=integrated_noise(rc_noise,1e-3Hz=>1e12Hz;quantity=:variance)
    @test rc_variance≈1.380649e-23*temperature/capacitance rtol=.02
    ...
    @circuit NoisyFollower() begin
        ...
        A1=opamp(positive,output,output,supply,gnd;
            model=BehavioralOpAmp(input_voltage_noise_density=5nV/sqrt(Hz),
    ...
    @test !isempty(noise_contributions(follower_noise;component=:A1,
        mechanism=:opamp_voltage))
    @test !isempty(noise_contributions(follower_noise;component=:A1,
        mechanism=:opamp_current))

    rectifier=noise(HalfWaveRectifier(),[1kHz];output=voltage(:vout))
    @test any(contribution->contribution.component===:D1&&
...
@testset "fixed-grid transient noise reproducibility" begin
    first_run=transient_noise(LowPass(),0s=>10μs;timestep=1μs,seed=0x1234,
        abstol=1e-15)
    second_run=transient_noise(LowPass(),0s=>10μs;timestep=1μs,seed=0x1234,
        abstol=1e-15)
    changed_seed=transient_noise(LowPass(),0s=>10μs;timestep=1μs,seed=0x1235,
        abstol=1e-15)
    ...
    @test_throws AnalysisValidationError transient_noise(LowPass(),0s=>10μs;
        timestep=3μs,seed=1)
end
