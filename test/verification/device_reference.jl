@testset "verification: nonlinear device references" begin
    temperature = 325.0; thermal = Amber._thermal_voltage(temperature)
    model = JunctionDiode(saturation_current = 2nA, ideality = 1.4, breakdown_voltage = 5V, breakdown_current = 1nA)
    @circuit BiasedDiode(; bias = 0.55V, model = model) begin
    ...
    expected_forward = model.saturation_current * expm1(0.55V / (thermal * model.ideality))
    @test current(forward, :D1)[1] ≈ expected_forward rtol = 2.0e-10
    ...
    @test current(reverse, :D1)[1] ≈ expected_reverse rtol = 2.0e-10

    transistor = GummelPoonBJT(saturation_current = 10fA, forward_beta = 120.0, reverse_beta = 2.0, early_voltage = 80V)
    @circuit VoltageBiasedBJT(; model = transistor) begin
        gnd = ground(); collector = node(); base = node(); emitter = node()
        VC = voltage_source(collector, gnd; dc = 3V); VB = voltage_source(base, gnd; dc = 0.68V); VE = voltage_source(emitter, gnd; dc = 0V)
        Q1 = npn(collector, base, emitter; model)
    end
    bjt = operating_point(VoltageBiasedBJT(); temperature)
    If = transistor.saturation_current * expm1(0.68V / thermal); Ir = transistor.saturation_current * expm1((0.68V - 3V) / thermal)
    ...
    @test current(bjt, :Q1, :collector)[1] ≈ expected_collector rtol = 2.0e-10
    @test current(bjt, :Q1, :base)[1] ≈ expected_base rtol = 2.0e-10
end

@testset "verification: behavioral op-amp reference" begin
    gain = 1.0e4; bandwidth = 1MHz
    @circuit FollowerReference() begin
        ...
        Input = voltage_source(input, gnd; dc = 0.1V, ac = 1V)
    ...
    @test voltage(follower, :output)[1] ≈ gain / (1 + gain) * 0.1V rtol = 2.0e-8
    response = small_signal(FollowerReference(), bandwidth => bandwidth; source = :Input)
    expected = gain / (1 + gain + im * gain)
    @test voltage(response, :output)[1] ≈ expected rtol = 2.0e-8
end
