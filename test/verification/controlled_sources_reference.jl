@testset "verification: controlled-source equations" begin
    @circuit ControlledReference() begin
        ...
        E = voltage_controlled_voltage_source(control, gnd, vcvs_output, gnd; gain = 4.0)
        F = current_controlled_current_source(Sense, cccs_output, gnd; gain = 5.0)
    ...
    sense_current = -2mA
    @test voltage(result, :vccs_output)[1] ≈ -3V rtol = 1.0e-12
    @test voltage(result, :vcvs_output)[1] ≈ 8V rtol = 1.0e-12
    @test voltage(result, :cccs_output)[1] ≈ -5 * 100Ω * sense_current rtol = 1.0e-12
    @test voltage(result, :ccvs_output)[1] ≈ 2kΩ * sense_current rtol = 1.0e-12
end
