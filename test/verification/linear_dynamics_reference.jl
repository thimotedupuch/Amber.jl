@testset "verification: RC, RL, and RLC references" begin
    ...
    @test maximum(abs.(voltage(rc,:output).-exact_rc.(rc.axis)))<2e-3
    ac=small_signal(RCReference(),10Hz=>100kHz;points=41,source=:V1)
    expected_ac=1 ./ (1 .+ im*2π.*ac.axis.*τ)
    @test relative_error(voltage(ac,:output),expected_ac)<1e-11
    ...
    @test maximum(abs.(voltage(rlc,:output).-exact_rlc.(rlc.axis)))<6e-3
end
