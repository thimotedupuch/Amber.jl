...
@circuit BiasedNPN() begin
    ...
    Q1 = npn(coll, base, emit; model = GummelPoonBJT(forward_beta = 150.0))
end

@circuit RealisticRC() begin
    gnd = ground(); a = node(); b = node()
    Vin = voltage_source(a, gnd; waveform = Sine(amplitude = 1V, frequency = 1kHz))
    R1 = resistor(a, b; value = 10kΩ, material = ThinFilm(), package = SMD0603(series_inductance = 0.6nH))
...
@circuit HalfWaveRectifier(; frequency = 50Hz, amplitude = 10V, load = 1kΩ, smoothing = 470μF) begin
    ...
    C1 = capacitor(vout, gnd; value = smoothing, esr = 180mΩ, leakage_resistance = 500kΩ, dielectric_absorption = DebyeBranches(time_constants = [20ms, 200ms, 2s], fractions = [0.015, 0.006, 0.002]))
...
function RLGCLine(; sections = 5)
    ...
    for k in 1:sections
        middle = node!(builder, (:middle, k)); add!(builder, resistor(x[k - 1], middle; value = 0.01Ω); name = (:R, k))
    ...
    observe!(builder, voltage(x[0]), voltage(x[sections]))
