@testset "behavioral current source" begin
    @circuit ExponentialDecay begin
        ...
        behavioral_current_source(((state,gnd),),state,gnd;
            current=(x,t)->x[1],gradient=(x,t)->(1.,0.,0.,0.))
        initial_voltage(Cstate,1.0)
        observe(state)
    end
    result=transient(ExponentialDecay(),0s=>1s;max_step=1ms)
    @test result.stats[:converged]
    @test voltage(result,:state)[end] ≈ exp(-1) rtol=2e-3
    ...
    @circuit TwoControlProduct begin
        ...
        behavioral_current_source(((x,gnd),(y,gnd)),output,gnd;
            current=(v,t)->v[1]*v[2],gradient=(v,t)->(v[2],v[1],0.,0.))
        observe(output)
    end
...
@testset "behavioral voltage source" begin
    @circuit SquaringVoltageSource begin
        ...
        behavioral_voltage_source(((input,gnd),),output,gnd;
            voltage=(v,t)->v[1]^2,gradient=(v,t)->(2v[1],0.,0.,0.))
    ...
    @circuit ArbitraryVoltageGenerator begin
        ...
        behavioral_voltage_source((),output,gnd;
            voltage=(v,t)->sin(2π*t),gradient=(v,t)->(0.,0.,0.,0.))
        resistor(output,gnd;value=1.0)
        observe(output)
    end
    result=transient(ArbitraryVoltageGenerator(),0s=>0.25s;max_step=0.5ms)
    @test result.stats[:converged]
    @test voltage(result,:output)[end] ≈ 1.0 atol=1e-5
end
