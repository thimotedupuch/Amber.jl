...
@testset "level-1 MOSFET and CMOS" begin
    model = Level1MOSFET(
        threshold_voltage = 1V, transconductance = 2mA / V^2,
        channel_length_modulation = 0.02 / V, body_effect = 0.0,
    ...
    expected = 0.5model.transconductance * (2.5V - model.threshold_voltage)^2 * (1 + model.channel_length_modulation * 3V)
    @test current(nresult, :M1)[1] ≈ expected rtol = 2.0e-10
    ...
    @test current(presult, :M1)[1] ≈ -expected rtol = 2.0e-10
    ...
    @test voltage(high, :output)[1] < 0.01V
    ...
    previous = point .- range(1.0e-7, 4.0e-7; length = compiled.n); α = 2.0e5
    _, analytic = Amber.residual_jacobian(compiled, point, previous, 0.0, α)
    numerical = Amber._jacobian(z -> Amber.residual(compiled, z, α .* (z .- previous), 0.0), point)
    @test norm(Matrix(analytic) - numerical) / max(norm(numerical), eps()) < 2.0e-5
end
