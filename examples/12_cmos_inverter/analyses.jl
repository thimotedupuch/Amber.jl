...
vtc=sweep(inverter,"Input.dc"=>range(0V,5V;length=401);
...
function measure_switching(step)
    result=transient(switching,0s=>210ns;event_mode=:exact,max_step=step,
...
relative_change=NamedTuple{(:tphl,:tplh,:energy)}(Tuple(
    abs(getproperty(metrics,k)-getproperty(coarse_metrics,k))/abs(getproperty(metrics,k))
    for k in (:tphl,:tplh,:energy)))
@assert all(<(.01),values(relative_change)) "Refine further: delay/energy changed by more than 1%"
# A single selected edge can also be measured explicitly.
delay=propagation_delay(waveforms;input=:input,output=:output,threshold=2.5V,
    input_edge=:rising,output_edge=:falling,window)
@assert isapprox(delay,metrics.tphl)
println((noise_margins=margins.measurements,delay_s=metrics.tphl,
