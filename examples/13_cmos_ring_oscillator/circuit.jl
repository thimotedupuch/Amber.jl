...
@subcircuit RingStage(
    input, output, supply, reference; load_capacitance = 5pF,
...
@circuit CMOSRingOscillator(;
    supply_voltage = 5V, load_capacitance = 5pF,
    startup_offset = 50mV
) begin
    ...
    First = RingStage(
        stage5, stage1, supply, gnd; load_capacitance,
        initial_output = supply_voltage / 2 + startup_offset
    )
    Second = RingStage(
        stage1, stage2, supply, gnd; load_capacitance,
        initial_output = supply_voltage / 2 - startup_offset
    )
    Third = RingStage(
        stage2, stage3, supply, gnd; load_capacitance,
        initial_output = supply_voltage / 2 + startup_offset
    )
    Fourth = RingStage(
        stage3, stage4, supply, gnd; load_capacitance,
        initial_output = supply_voltage / 2 - startup_offset
    )
    Fifth = RingStage(
        stage4, stage5, supply, gnd; load_capacitance,
        initial_output = supply_voltage / 2 + startup_offset
    )
    observe(
        voltage(stage1), voltage(stage2), voltage(stage3), voltage(stage4),
