...
function ConductanceCrossbar(;
    ...
    for column in 1:columns
        add!(
            c, voltage_source(input_nodes[column], gnd; dc = inputs[column]);
    ...
    for row in 1:rows, column in 1:columns
        add!(
            c, conductance(
                input_nodes[column], summing_nodes[row];
    ...
    for row in 1:rows
        add!(
            c, opamp(
                gnd, summing_nodes[row], output_nodes[row], positive_rail, negative_rail;
                model = BehavioralOpAmp(
                    dc_gain = 120dB, gain_bandwidth = 10MHz,
                    output_resistance = 1Ω
                )
            ); name = Symbol(:TIA_, row)
        )
        add!(
            c, resistor(output_nodes[row], summing_nodes[row]; value = feedback_resistance);
