...
@circuit SampleAndHold(;sampling_frequency=100kHz,hold_capacitance=1nF) begin
    ...
    Clock=voltage_source(clk,gnd;waveform=Pulse(low=0V,high=5V,frequency=sampling_frequency,duty_cycle=.15,rise=2ns,fall=2ns))
    ...
    Chold=capacitor(hold,gnd;value=hold_capacitance,leakage_resistance=1GΩ,dielectric_absorption=DebyeBranches(time_constants=[100μs,2ms],fractions=[.003,.001]))
    Buffer=opamp(hold,out,out,vdd,vss;model=BehavioralOpAmp(dc_gain=100dB,gain_bandwidth=20MHz,input_bias_current=5pA,input_capacitance=2pF,slew_rate=10V/μs))
