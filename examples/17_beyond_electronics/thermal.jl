...
@circuit ElectroThermalHeater(; supply=12V, resistance_ambient=12.0,
        temperature_coefficient=0.0039, ambient=293.15) begin
    ...
    behavioral_current_source(((electrical,gnd),(heater_temperature,gnd)),electrical,gnd;
        current=(x,t)->x[1]/resistance(x[2]),
        gradient=(x,t)->(inv(resistance(x[2])),
            -x[1]*resistance_ambient*temperature_coefficient/resistance(x[2])^2,0.,0.))
    behavioral_current_source(((electrical,gnd),(heater_temperature,gnd)),heater_temperature,gnd;
        current=(x,t)->-x[1]^2/resistance(x[2]),
        gradient=(x,t)->(-2x[1]/resistance(x[2]),
            x[1]^2*resistance_ambient*temperature_coefficient/resistance(x[2])^2,0.,0.))
