...
@circuit JosephsonJunction(; bias = 1.15, damping = 0.25) begin
    ...
    resistor(phase, gnd; value = 1.0e15); resistor(phase_velocity, gnd; value = 1.0e15)
    ...
    behavioral_current_source(
        controls, phase, gnd; current = (x, t) -> -x[2],
        gradient = (x, t) -> (0.0, -1.0, 0.0, 0.0)
    )
    behavioral_current_source(
        controls, phase_velocity, gnd;
        current = (x, t) -> sin(x[1]) + damping * x[2] - applied_bias(t),
        gradient = (x, t) -> (cos(x[1]), damping, 0.0, 0.0)
    )
