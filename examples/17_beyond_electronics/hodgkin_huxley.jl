...
@circuit HodgkinHuxley(; stimulus=10.0) begin
    ...
    for state in (membrane,m_gate,h_gate,n_gate); resistor(state,gnd;value=1e15) end
    ...
    dynamics(x, _) = begin
        ...
        ina=120m^3*h*(1e3v-115); ik=36n^4*(1e3v+12); il=0.3*(1e3v-10.613)
        ((stimulus-ina-ik-il), 1e3*(am*(1-m)-bm*m),
         1e3*(ah*(1-h)-bh*h), 1e3*(an*(1-n)-bn*n))
    end
    function jacobian_row(row, x, t)
        epsilon=1e-6; base=dynamics(x,t)[row]
        ntuple(j -> begin
    ...
    behavioral_current_source(controls, membrane, gnd;
        current=(x,t)->-dynamics(x,t)[1], gradient=(x,t)->.-jacobian_row(1,x,t))
    behavioral_current_source(controls, m_gate, gnd;
        current=(x,t)->-dynamics(x,t)[2], gradient=(x,t)->.-jacobian_row(2,x,t))
    behavioral_current_source(controls, h_gate, gnd;
        current=(x,t)->-dynamics(x,t)[3], gradient=(x,t)->.-jacobian_row(3,x,t))
    behavioral_current_source(controls, n_gate, gnd;
