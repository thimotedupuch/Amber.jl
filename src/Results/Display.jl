...
function Base.show(io::IO, r::EngineeringReport)
    ...
    return if get(r, :detailed, false)&&stats !== nothing
        print(io, "\n  Solver statistics:")
...
function _result_status(r)
    ...
    return get(r.stats, :converged, false) ? "solver converged" : "solver did not converge"
end

function Base.show(io::IO, r::_InteractiveResult)
    count = length(_result_axis(r))
    return print(io, _result_name(r), " result (", _result_status(r), ", ", count, count == 1 ? " sample)" : " samples)")
end

function _display_excerpt(message)
    text = replace(string(message), '\n' => ' ')
    return length(text) > 180 ? first(text, 180) * "…" : text
end
...
function Base.show(io::IO, ::MIME"text/plain", r::_InteractiveResult)
    ...
    return if !get(r.stats, :converged, false)
...
function Base.show(io::IO, r::_InteractiveStudy)
    return print(
        io, r isa SweepResult ? "Sweep" : "Monte Carlo", " result (",
...
function Base.show(io::IO, design::CircuitDesign)
    info = summary(design)
    return print(
        io, "CircuitDesign ", info.name, " (", info.nets, " root nets, ",
...
function Base.show(io::IO, compiled::CompiledCircuit)
    return print(
        io, "CompiledCircuit ", _name(compiled.design.names, compiled.design.name),
        " (", compiled.n, " unknowns)"
    )
end
