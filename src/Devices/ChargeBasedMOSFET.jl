...
_MOSJet{G, H}(x::Real) where {G, H} = _MOSJet(Float64(x), ntuple(_ -> 0.0, G), ntuple(_ -> 0.0, H))
_MOSJet(x::Real) = _MOSJet{4, 16}(x)
_mos_variable(x, i, ::Val{O} = Val(2)) where {O} =
    _MOSJet(Float64(x), ntuple(j -> Float64(i == j), O == 0 ? 0 : 4), ntuple(_ -> 0.0, O == 2 ? 16 : 0))
...
function Base.:*(a::_MOSJet, b::_MOSJet)
    return _MOSJet(
        a.value * b.value, a.gradient .* b.value .+ b.gradient .* a.value,
...
function _mos_lift(x::_MOSJet, value, first, second)
    return _MOSJet(
        value, x.gradient .* first, ntuple(length(x.hessian)) do k
            i = (k - 1) ÷ 4 + 1; j = (k - 1) % 4 + 1
            first * x.hessian[k] + second * x.gradient[i] * x.gradient[j]
        end
    )
end
Base.sqrt(x::_MOSJet) = _mos_lift(x, sqrt(x.value), 0.5 / sqrt(x.value), -0.25 / x.value^1.5)
...
Base.:^(a::_MOSJet, n::Integer) = n == 0 ? typeof(a)(1.0) : n == 1 ? a :
...
function _mos_inversion(v::_MOSJet)
    ...
    if x < -36
        ...
        for _ in 1:20
            ...
            abs(step) < 2.0e-15 && break
        end
        q = exp(y)
    end
    return _mos_lift(v, q, q / (1 + 2q), q / (1 + 2q)^3)
end
...
function _mos_junction_charge(v, capacitance, model)
    ...
    if v.value <= transition
        ...
        first = capacitance * base^(-grading)
        value = at_transition + first * (v.value - transition); second = 0.0
    end
    return _mos_lift(v, value, first, second)
end
...
function _mos_junction_current(v, saturation, ut)
    ...
    return _mos_lift(v, saturation * value, saturation * slope / ut, saturation * curvature / ut^2)
end

function _charge_mos_evaluate(model::ChargeBasedMOSFET, kind, voltages; temperature = 300.0, order = Val(2))
    ...
    bulk_charge = typeof(channel)(0.0)
    charges = [drain_charge, gate_charge, source_charge, bulk_charge]
    currents = [channel, typeof(channel)(0.0), -channel, typeof(channel)(0.0)]
    for (other, capacitance) in (
            (1, model.gate_drain_overlap * model.width),
    ...
    for (terminal, area, perimeter) in (
            (1, model.drain_area, model.drain_perimeter),
            (3, model.source_area, model.source_perimeter),
        )
        junction_voltage = vb - (vd, vg, vs, vb)[terminal]
        capacitance = model.multiplicity * (
            model.junction_capacitance_density * area +
...
function _mosfet_channel(model::ChargeBasedMOSFET, kind::Symbol, vd, vg, vs, vb; temperature = 300.0)
    channel = _charge_mos_evaluate(model, kind, (vd, vg, vs, vb); temperature, order = Val(1)).channel
    return channel.value, channel.gradient
end
...
"""
function terminal_charges(model::ChargeBasedMOSFET, kind, vd, vg, vs, vb; temperature = 300.0)
    evaluated = _charge_mos_evaluate(model, kind, (vd, vg, vs, vb); temperature, order = Val(0))
    return NamedTuple{(:drain, :gate, :source, :bulk)}(map(q -> q.value, evaluated.charges))
end
...
"""
function mosfet_operating_point(model::ChargeBasedMOSFET, kind, vd, vg, vs, vb; temperature = 300.0)
