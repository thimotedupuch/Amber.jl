...
"""
with_model_parameter(model, name::Symbol, value) = throw(
    ArgumentError(
...
Base.@kwdef struct Step <: AbstractWaveform
    low::Float64 = 0.0; high::Float64 = 1.0; at::Float64 = 0.0; rise::Float64 = 0.0
end
Base.@kwdef struct Sine <: AbstractWaveform
    amplitude::Float64 = 1.0; frequency::Float64 = 1.0; phase::Float64 = 0.0; offset::Float64 = 0.0
end
Base.@kwdef struct Pulse <: AbstractWaveform
    low::Float64 = 0.0; high::Float64 = 1.0; frequency::Float64 = 1.0; duty_cycle::Float64 = 0.5
    rise::Float64 = 0.0; fall::Float64 = 0.0; delay::Float64 = 0.0
end
...
function _validate_waveform(w::Step)
    isfinite(w.at)&&isfinite(w.rise)&&w.rise >= 0||throw(AnalysisValidationError("step time and rise time must be finite, with non-negative rise"))
    return w
end
function _validate_waveform(w::Sine)
    isfinite(w.frequency)&&w.frequency >= 0||throw(AnalysisValidationError("sine frequency must be finite and non-negative"))
    return w
end
...
const _RESISTOR_MATERIAL_NAMES = (
    :ThinFilm, :ThickFilm, :MetalFilm, :CarbonFilm,
    :CarbonComposition, :MetalFoil, :Wirewound,
)
const _PASSIVE_PACKAGE_NAMES = (
    :SMD0201, :SMD0402, :SMD0603, :SMD0805, :SMD1206,
    :SMD1210, :SMD2010, :SMD2512, :Axial, :Radial, :PassivePackage,
)
const _CAPACITOR_DIELECTRIC_NAMES = (
    :C0G, :X7R, :X5R, :Polypropylene, :Polyester,
...
for (T, defaults) in (
        (
            (
                name, :(;
                    tc1 = 0.0, temperature_coefficient = 0.0, voltage_coefficient = 0.0,
                    excess_noise_coefficient = 0.0, excess_current_exponent = 2.0,
                    excess_frequency_exponent = 1.0, excess_reference_frequency = 1.0,
                ),
            )
                for name in _RESISTOR_MATERIAL_NAMES
        )...,
        (
            (name, :(; series_inductance = 0.0, parallel_capacitance = 0.0, esr = 0.0, esl = 0.0))
                for name in _PASSIVE_PACKAGE_NAMES
        )...,
        (
            (name, :(; loss_tangent = 0.0, reference_frequency = 0.0))
                for name in _CAPACITOR_DIELECTRIC_NAMES
        )...,
        (:DebyeBranches, :(; time_constants = Float64[], fractions = Float64[])),
        (
            :JunctionDiode, :(;
                saturation_current = 1.0e-12, ideality = 1.2, series_resistance = 0.0,
                junction_capacitance = 0.0, junction_potential = 0.7, grading_coefficient = 0.5, transit_time = 0.0,
                breakdown_voltage = Inf, breakdown_current = 1.0e-3, flicker_coefficient = 0.0,
                flicker_current_exponent = 1.0, flicker_frequency_exponent = 1.0,
                flicker_reference_frequency = 1.0,
            ),
        ),
        (
            :GummelPoonBJT, :(;
                saturation_current = 1.0e-14, forward_beta = 100.0, reverse_beta = 1.0,
                early_voltage = 100.0, base_resistance = 0.0, cbe_zero_bias = 0.0, cbc_zero_bias = 0.0,
                transit_time = 0.0, flicker_coefficient = 0.0, flicker_current_exponent = 1.0,
                flicker_frequency_exponent = 1.0, flicker_reference_frequency = 1.0,
            ),
        ),
        (
            :Level1MOSFET, :(;
                threshold_voltage = 0.7, transconductance = 1.0e-3,
                channel_length_modulation = 0.0, body_effect = 0.0, surface_potential = 0.6,
                gate_source_capacitance = 0.0, gate_drain_capacitance = 0.0, gate_bulk_capacitance = 0.0,
                channel_thermal_coefficient = 2 / 3, flicker_coefficient = 0.0,
                flicker_current_exponent = 2.0, flicker_frequency_exponent = 1.0,
                flicker_reference_frequency = 1.0, induced_gate_noise_coefficient = 0.0,
                gate_channel_correlation = 0.0 + 0.0im,
            ),
        ),
        (
            :ChargeBasedMOSFET, :(;
                threshold_voltage = 0.7, slope_factor = 1.3, channel_length_modulation = 0.0,
                mobility = 0.04, oxide_capacitance = 5.0e-3, width = 1.0e-6, length = 1.0e-6,
                multiplicity = 1.0, reference_temperature = 300.0, threshold_temperature_coefficient = -1.0e-3,
                mobility_temperature_exponent = -1.5, gate_source_overlap = 0.0, gate_drain_overlap = 0.0,
                gate_bulk_capacitance = 0.0, drain_area = 0.0, source_area = 0.0,
                drain_perimeter = 0.0, source_perimeter = 0.0, junction_capacitance_density = 1.0e-3,
                junction_sidewall_capacitance = 1.0e-10, junction_saturation_current_density = 1.0e-6,
                junction_potential = 0.7, junction_grading = 0.5,
                channel_thermal_coefficient = 1.0, flicker_coefficient = 0.0,
                flicker_current_exponent = 2.0, flicker_frequency_exponent = 1.0,
                flicker_reference_frequency = 1.0, induced_gate_noise_coefficient = 0.0,
                gate_channel_correlation = 0.0 + 0.0im,
            ),
        ...
        (
            :BehavioralOpAmp, :(;
                dc_gain = 1.0e5, gain_bandwidth = 1.0e6, slew_rate = Inf,
                output_resistance = 10.0, output_current_limit = Inf, input_offset = 0.0,
                input_voltage_noise_density = 0.0, input_voltage_flicker_corner = 0.0,
                input_voltage_flicker_exponent = 1.0, positive_input_current_noise_density = 0.0,
                negative_input_current_noise_density = 0.0, input_current_flicker_corner = 0.0,
                input_current_flicker_exponent = 1.0, voltage_current_noise_correlation = 0.0 + 0.0im,
                saturation_recovery = 0.0, input_bias_current = 0.0, input_capacitance = 0.0,
            ),
        ),
        (:VoltageControlledSwitch, :(; threshold = 0.5, ron = 1.0, roff = 1.0e12, charge_injection = 0.0, clock_feedthrough = 0.0)),
        (:EventSwitch, :(; threshold = 0.5, ron = 1.0, roff = 1.0e12, charge_injection = 0.0, clock_feedthrough = 0.0)),
        (:SmoothSwitch, :(; threshold = 0.5, transition = 0.05, ron = 1.0, roff = 1.0e12, charge_injection = 0.0, clock_feedthrough = 0.0)),
    )
    ...
    @eval begin
        ...
        function $T(; kw...)
            ...
            return _validate_model_parameters($T((; defaults..., kw...)))
        end
        ...
        function with_model_parameter(x::$T, name::Symbol, value)
            ...
            return _validate_model_parameters($T(merge(x.data, NamedTuple{(name,)}((value,)))))
        end
    end
end

struct IdealResistor
    value::Float64
end
struct IdealCapacitor
    value::Float64
end
...
function _unsupported_model_effects(model, neutral)
    for (name, value) in pairs(neutral)
        getproperty(model, name) == value||throw(
            ArgumentError(
...
function _validate_model_parameters(model::AbstractResistorMaterial)
    _unsupported_model_effects(model, (; tc1 = 0.0, temperature_coefficient = 0.0, voltage_coefficient = 0.0))
...
function _validate_model_parameters(model::AbstractCapacitorDielectric)
    ...
    frequency = get(model.data, :reference_frequency, 0.0) # Legacy lossless C0G snapshots.
    _finite_nonnegative_parameter(:reference_frequency, frequency)
    model.loss_tangent > 0 && frequency == 0 && throw(
        ArgumentError(
...
function _validate_model_parameters(model::GummelPoonBJT)
    ...
    return _unsupported_model_effects(model, (; transit_time = 0.0))
end
function _validate_model_parameters(model::BehavioralOpAmp)
    ...
    return _unsupported_model_effects(model, (; slew_rate = Inf, output_current_limit = Inf, saturation_recovery = 0.0))
end
...
function _validate_model_parameters(model::ChargeBasedMOSFET)
    ...
    for name in (
            :mobility, :oxide_capacitance, :width, :length, :multiplicity,
    ...
    for name in (
            :channel_length_modulation, :gate_source_overlap, :gate_drain_overlap, :gate_bulk_capacitance,
...
const _PASSIVE_MODELS = Tuple(
    getfield(@__MODULE__, name) for name in
