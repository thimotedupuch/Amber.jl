...
function zener(a,b;breakdown_voltage=5.1,breakdown_current=1e-3,kw...)
    diode(a,b;model=JunctionDiode(;breakdown_voltage=_positive(:breakdown_voltage,breakdown_voltage),
...
schottky(a,b;kw...)=diode(a,b;model=JunctionDiode(;merge((saturation_current=1e-8,ideality=1.05),NamedTuple(kw))...))
"""Electrical LED junction (no optical output), with illustrative defaults."""
led(a,b;kw...)=diode(a,b;model=JunctionDiode(;merge((saturation_current=1e-20,ideality=2.),NamedTuple(kw))...))

"""Photodiode with constant photocurrent from cathode to anode, in amperes."""
function photodiode(a,b;photocurrent=0.,model=JunctionDiode())
    _component(:photodiode,a,b;photocurrent=_nonnegative(:photocurrent,photocurrent),model)
end
"""Single-junction solar cell with photocurrent and finite shunt resistance."""
function solar_cell(a,b;photocurrent=1.,shunt_resistance=1e6,model=JunctionDiode())
    _component(:solar_cell,a,b;photocurrent=_nonnegative(:photocurrent,photocurrent),
...
function _catalog_source(kind,controls,a,b,parameters;voltage=false)
    ...
    voltage ? behavioral_voltage_source(controls,a,b;voltage=law,gradient=CatalogGradient(law)) :
...
function analog_multiplier(xp,xn,yp,yn,a,b;gain=1.,offset=0.)
    all(isfinite,(gain,offset))||throw(ArgumentError("gain and offset must be finite"))
    _catalog_source(:multiplier,((xp,xn),(yp,yn)),a,b,(gain,offset);voltage=true)
end
"""Smooth voltage limiter: midpoint + halfspan*tanh(gain*(Vin-offset)/halfspan)."""
function voltage_limiter(cp,cn,a,b;low=-1.,high=1.,gain=1.,offset=0.)
    all(isfinite,(low,high,gain,offset))&&high>low||throw(ArgumentError("finite parameters and high > low required"))
    _catalog_source(:limiter,((cp,cn),),a,b,(low,high,gain,offset);voltage=true)
end
"""Memoryless smooth comparator with fixed output levels; no delay or hysteresis."""
comparator(cp,cn,a,b;low=0.,high=5.,gain=1e4,offset=0.)=voltage_limiter(cp,cn,a,b;low,high,gain,offset)
"""Smooth voltage-controlled resistor; positive control lowers resistance toward `rmin`."""
function voltage_controlled_resistor(a,b,cp,cn;rmin=1.,rmax=1e6,threshold=0.,transition=1.)
...
function thermistor(a,b;rnom=1e4,beta=3950.,temperature=300.,nominal_temperature=298.15)
    ...
    resistor(a,b;value=_positive(:resistance,resistance))
end
"""N-channel square-law JFET, drain/gate/source; negative pinch-off voltage, no gate junction or noise."""
function njfet(d,g,s;idss=1e-3,pinch_off=-2.,channel_length_modulation=0.)
    _jfet(d,g,s,idss,pinch_off,channel_length_modulation,1.)
end
"""P-channel square-law JFET; use the same negative normalized pinch-off as `njfet`."""
function pjfet(d,g,s;idss=1e-3,pinch_off=-2.,channel_length_modulation=0.)
    _jfet(d,g,s,idss,pinch_off,channel_length_modulation,-1.)
end
function _jfet(d,g,s,idss,vp,lambda,polarity)
    ...
    _catalog_source(:jfet,((d,s),(g,s)),d,s,(idss,vp,lambda,polarity))
end
"""Three-terminal potentiometer (top, wiper, bottom); position is fraction measured from bottom."""
function potentiometer(top,wiper,bottom;resistance=1e4,position=.5)
    _positive(:resistance,resistance)
    0<position<1||throw(ArgumentError("position must be strictly between zero and one"))
    _component(:potentiometer,top,wiper,bottom;resistance,position)
end
"""Ideal lossless transformer; V(primary)=ratio*V(secondary), positive terminals dotted."""
function ideal_transformer(pp,pn,sp,sn;ratio=1.)
    _component(:ideal_transformer,pp,pn,sp,sn;ratio=_positive(:ratio,ratio))
end
...
function crystal(a,b;motional_resistance=10.,motional_inductance=.01,motional_capacitance=1e-12,shunt_capacitance=5e-12)
    foreach(pair->_positive(pair...),((:motional_resistance,motional_resistance),(:motional_inductance,motional_inductance),
        (:motional_capacitance,motional_capacitance),(:shunt_capacitance,shunt_capacitance)))
    _component(:crystal,a,b;motional_resistance,motional_inductance,motional_capacitance,shunt_capacitance)
end
"""Lumped RLGC line (input, output, reference); R,L,G,C are TOTAL line values, split into pi sections."""
function transmission_line(input,output,reference;resistance=1.,inductance=1e-6,conductance=0.,capacitance=1e-10,sections=10)
    ...
    _component(:transmission_line,input,output,reference;resistance,inductance,conductance,capacitance,sections)
end
...
function _add_catalog!(builder,component,name)
    ...
    if k in (:photodiode,:solar_cell)
        ...
        if model.series_resistance>0
            ...
            model=with_model_parameter(model,:series_resistance,0.)
        end
    ...
    elseif k===:transmission_line
        ...
        for i in 1:p.sections
            ...
            handle=add(inductor(previous,next;value=p.inductance/p.sections,
                winding_resistance=p.resistance/p.sections),i==1 ? "" : "section$(i)_l")
            i==1&&(main=handle)
