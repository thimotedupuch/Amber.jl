...
function _mosfet_component(
        kind, d, g, s, b; model = Level1MOSFET(), width = nothing, length = nothing,
    ...
    return _component(kind, d, g, s, b; model, kw...)
end
...
function _mosfet_channel(model::Level1MOSFET, kind::Symbol, vd, vg, vs, vb; temperature = 300.0)
    ...
    body_slope = root_argument > eps(Float64) ? data.body_effect / (2root) : 0.0
    threshold = data.threshold_voltage + data.body_effect * (root - sqrt(twice_phi))
    overdrive = ug - low - threshold
    overdrive <= 0&&return (zero(promote_type(typeof(vd), Float64)), (0.0, 0.0, 0.0, 0.0))
    ...
    return polarity * normalized, derivatives
end
...
function _junction_depletion(model::JunctionDiode, voltage)
    ...
    transition = 0.9 * potential
    base(v) = abs(1 - grading) < 1.0e-12 ? -capacitance * potential * log1p(-v / potential) : capacitance * potential / (1 - grading) * (1 - (1 - v / potential)^(1 - grading))
    return if voltage <= transition
        base(voltage)
    else
...
function charge(model::JunctionDiode, voltage::Real; temperature = 300.0)
    data = getfield(model, :data); vt = _thermal_voltage(temperature) * data.ideality
    forward, _ = _limited_exponential(voltage / vt)
    return _junction_depletion(model, voltage) + data.transit_time * data.saturation_current * forward
end

# dC/dv, using exactly the same continuation as charge and capacitance.
function _capacitance_slope(model::JunctionDiode, voltage; temperature = 300.0)
    data = getfield(model, :data); potential = data.junction_potential
    depletion = voltage < 0.9potential ? data.junction_capacitance * data.grading_coefficient /
        potential * (1 - voltage / potential)^(-data.grading_coefficient - 1) : 0.0
    vt = _thermal_voltage(temperature) * data.ideality; argument = voltage / vt
    diffusion = -80 < argument < 80 ? data.transit_time * data.saturation_current * exp(argument) / vt^2 : 0.0
    return depletion + diffusion
end

function differential_capacitance(model::JunctionDiode, voltage::Real; temperature = 300.0)
    data = getfield(model, :data); potential = data.junction_potential; transition = 0.9 * potential
    depletion = data.junction_capacitance == 0 ? 0.0 : data.junction_capacitance * (1 - min(voltage, transition) / potential)^(-data.grading_coefficient)
    vt = _thermal_voltage(temperature) * data.ideality; _, slope = _limited_exponential(voltage / vt)
    return depletion + data.transit_time * data.saturation_current * slope / vt
end
