...
const _COMPONENT_KEYWORDS = Dict(
    ...
    :capacitor => (
        :value, :package, :dielectric, :esr, :esl, :leakage_resistance,
...
function _component(k, ns...; kw...)
    ...
    return PrimitiveDraft{k, typeof(ns), typeof((; kw...))}(ns, (; kw...))
end

voltage_source(a, b; dc = 0.0, ac = 0.0, waveform = nothing, kw...) = _component(:voltage_source, a, b; dc, ac, waveform, kw...)
current_source(a, b; dc = 0.0, ac = 0.0, waveform = nothing, kw...) = _component(:current_source, a, b; dc, ac, waveform, kw...)

transconductance(control_p, control_n, output_p, output_n; gm = 1.0, kw...) = _component(:vccs, control_p, control_n, output_p, output_n; gm, kw...)
voltage_controlled_voltage_source(control_p, control_n, output_p, output_n; gain = 1.0, kw...) = _component(:vcvs, control_p, control_n, output_p, output_n; gain, kw...)
...
current_controlled_current_source(control, output_p, output_n; gain = 1.0, kw...) = _component(:cccs, output_p, output_n; control = _control_name(control), gain, kw...)
current_controlled_voltage_source(control, output_p, output_n; transresistance = 1.0, kw...) = _component(:ccvs, output_p, output_n; control = _control_name(control), transresistance, kw...)
