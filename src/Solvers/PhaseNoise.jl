...
noise_density(result::PhaseNoiseResult) = sqrt.(max.(result.phase_noise_ratio, 0.0))
...
function _phase_sensitivity(pss, times, values, temperature)
    ...
    abs(normalization) > sqrt(eps(Float64))||throw(
        AnalysisValidationError(
    ...
    for step in length(times):-1:1
        ...
        residual_adjoint = _solve_linear(
            systems[step]', adjoint_at_next,
            "oscillator discrete adjoint is singular at time $(extended_times[step + 1])"
        ) / h
    ...
    return waveform, multipliers[neutral_index]
end

function _phase_projection_spectrum(
        pss, times, values, sensitivity, temperature,
        frequencies
    )
    ...
    for time_index in eachindex(times)
        ...
        projections = ComplexF64[
            dot(sensitivity[:, time_index], source.injection)
                for source in sources
        ]
        for (frequency_index, frequency) in enumerate(frequencies)
            covariance = _instantaneous_source_covariance(
                sources, groups,
    ...
    return reference_sources, source_spectra
end

function phase_noise(
        pss::PSSResult, offset_specification; output, sidebands = -5:5,
    ...
    sources, source_projection = _phase_projection_spectrum(
        pss, times, values,
    ...
    periodic = periodic_noise(
        pss, offsets; output, input = nothing, output_harmonic = 1,
    ...
    return PhaseNoiseResult(
        offsets, _as_observable(output), single_sideband, dbc,
