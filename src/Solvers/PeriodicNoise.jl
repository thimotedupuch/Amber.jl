...
noise_density(result::PeriodicNoiseResult) = sqrt.(max.(result.output_psd, 0.0))
...
input_referred_noise_density(result::PeriodicNoiseResult) =
    ...
    sqrt.(max.(result.input_referred_psd, 0.0))
...
function _periodic_orbit_samples(pss)
    ...
    if length(times) > 1&&isapprox(
            last(times) - first(times), pss.period;
            rtol = 1.0e-8, atol = eps(Float64) * max(pss.period, 1.0)
        )
        sample_times = times[1:(end - 1)]
        sample_values = values[:, 1:(end - 1)]
        steps = diff(times)
        isempty(steps)||maximum(abs.(steps .- first(steps))) <=
            1.0e-8 * max(abs(first(steps)), eps(Float64))||
            throw(
...
function _periodic_jacobians(pss, temperature)
    ...
    return times, values, conductance, dynamics, inventories
end

function _fourier_coefficient(samples, q)
    count = length(samples)
    return sum(samples[index] * cis(-2π * q * (index - 1) / count) for index in 1:count) / count
end
...
function _periodic_coefficients(conductance, dynamics, sidebands)
    ...
    return Dict(
        q => (
            sparse(_fourier_coefficient(conductance, q)),
            sparse(_fourier_coefficient(dynamics, q)),
...
function _lifted_periodic_system(conductance, dynamics, sidebands, offset, period)
    ...
    return _lifted_periodic_system(coefficients, sidebands, offset, period)
end
...
function _lifted_periodic_system(coefficients, sidebands, offset, period)
    ...
    return sparse(rows, columns, entries, dimension, dimension)
end
...
function _instantaneous_source_cross_covariance(
        sources, groups, row_frequency,
        column_frequency, bias, time
    )
    source_index = Dict(source.id => index for (index, source) in enumerate(sources))
    row_psd = [_noise_source_psd(source, row_frequency, bias, time) for source in sources]
    column_psd = [
        _noise_source_psd(source, column_frequency, bias, time)
...
function _periodic_source_data(inventories, values, times, frequency)
    ...
    for time_index in eachindex(inventories)
        ...
        getfield.(sources, :id) == ids||throw(
            AnalysisValidationError(
                "periodic device noise inventory changed over the orbit"
            )
        )
        push!(
            covariances, _instantaneous_source_covariance(
                sources, groups, frequency,
                values[:, time_index], times[time_index]
            )
        )
    end
    return reference_sources, covariances
end

function _periodic_source_cross_data(
        inventories, values, times, row_frequency,
        column_frequency
    )
    reference_sources = first(inventories)[1]
    ids = getfield.(reference_sources, :id)
    ...
    for time_index in eachindex(inventories)
        ...
        push!(
            covariances, _instantaneous_source_cross_covariance(
                sources, groups,
...
function _periodic_noise_at_offset(
        adjoint_matrix, selector, output_block, sources, covariances,
    ...
    for (sideband_index, _) in enumerate(sidebands)
        ...
        for source_index in eachindex(sources)
            transfers[source_index, sideband_index] =
                ...
                (sideband_index == output_block ? direct[source_index] : 0.0)
        end
    end
    total = 0.0
    ...
    tolerance = eps(Float64) * max(sum(abs, allocations), 1.0)
    total >= -tolerance||throw(
        AnalysisValidationError(
            "periodic noise covariance produced a negative output PSD"
        )
    )
    return max(total, 0.0), allocations, max.(sideband_power, 0.0)
end

function periodic_noise(
        pss::PSSResult, offset_specification; output, input = nothing,
        output_harmonic = 0, sidebands = -5:5, temperature = pss.stats[:temperature],
        points = 100, scale = :log
    )
    get(pss.stats, :converged, false)||throw(
        ConvergenceError(
    ...
    all(diff(harmonics) .== 1)||throw(
        AnalysisValidationError(
            "sidebands must be a contiguous integer range"
        )
    )
    output_harmonic in harmonics||throw(
        AnalysisValidationError(
    ...
    reference_sources = first(inventories)[1]
    direct = Float64[
        _noise_output_direct(pss.orbit.compiled, output_observable, source)
    ...
    for (offset_index, offset) in enumerate(offsets)
        ...
        for row_harmonic in harmonics,column_harmonic in harmonics
            ...
            sources, covariance_by_pair[(row_harmonic, column_harmonic)] =
                _periodic_source_cross_data(
                inventories, values, times,
                row_frequency, column_frequency
            )
        end
        total, allocations, _ = _periodic_noise_at_offset(
            adjoints, selector,
        ...
        for sideband_index in eachindex(harmonics)
            if sideband_index == output_block
                ...
                sideband_total, _, _ = _periodic_noise_at_offset(
                    adjoints, selector,
        ...
        if length(harmonics) >= 3&&output_harmonic in harmonics[2:(end - 1)]
            ...
            nested_system = _lifted_periodic_system(
                coefficients,
                nested_harmonics, offset, pss.period
            )
            nested_adjoints = _periodic_adjoint_matrix(
                nested_system, selector,
                length(nested_harmonics), offset
            )
            nested_block = findfirst(==(output_harmonic), nested_harmonics)
            nested_total, _, _ = _periodic_noise_at_offset(
                nested_adjoints, selector,
    ...
    return PeriodicNoiseResult(
        offsets, output_observable,
