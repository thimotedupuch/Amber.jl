function _apply_initial_conditions!(z,cc)
    ...
    function constraint(a,b,value)
        ...
        push!(edges[b+1],(a,value)); push!(edges[a+1],(b,-value))
    end
    has_initial=false
    ...
    for batch in cc.parameters.batches
        ...
        for device in eachindex(batch.parameters)
            p=batch.parameters[device]
            iszero(get(p,:series_resistance,0.))||continue
            constraint(Int(batch.terminals[1][device]),Int(batch.terminals[2][device]),Float64(get(p,:dc,0.)))
        end
    end
    ...
    for root in 0:cc.n
        ...
        for node in component
            for (neighbor,delta) in edges[node+1]
                ...
                if visited[neighbor+1]
                    isapprox(offsets[neighbor+1],expected;atol=1e-12,rtol=1e-10)||
        ...
        shift=root==0 ? 0. : sum(z[node]-offsets[node+1] for node in component)/length(component)
        for node in component
...
function transient(c,p::Pair;overrides=nothing,kw...)
    ...
    _transient(updated,p;overrides,kw...)
end
...
function _transient(c,p::Pair;saveat=nothing,max_step=nothing,method=:bdf2,adaptive=nothing,
        ...
        temperature=300.,solver=SolverOptions(),integration=IntegrationOptions(),
    ...
    function step(previous,time,h,qhistory;guess=previous,alpha=inv(h))
        _newton(cc,guess,previous,time,alpha;storage_history=qhistory,
    ...
    while last(times)<t1
        ...
        h=endpoint-time
        h>0||begin failure_reason="transient cannot advance time"; push!(failed_steps,length(times)+1); break end
        ...
        if event_mode===:exact
            ...
            if !isempty(crossings)
                ...
                event_tolerance=max(minimum_step,h*1e-9)
                for _ in 1:48
        ...
        if !good||!isfinite(error)||error>1
            ...
            if use_adaptive&&h>minimum_step
                dt=max(minimum_step,h*.5); continue
            end
        ...
        restart=event
        if use_adaptive
            factor=error==0 ? 2. : clamp(.9*error^(-.5),.25,2.)
            dt=min(maximum_step,max(minimum_step,h*factor))
        end
    end
    analysis=Transient(t0=>t1;saveat,max_step,method,adaptive=use_adaptive,temperature,
        solver,initial=initial isa AbstractVector ? Float64.(initial) : initial,event_mode,integration,failure_policy,overrides)
    stats=Dict{Symbol,Any}(:converged=>failure_reason===nothing,:iterations=>total_iterations,
    ...
    SimulationResult(cc,analysis,times,values,stats)
end
