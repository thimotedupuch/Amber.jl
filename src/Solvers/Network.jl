struct Port
    ...
    function Port(positive, negative; reference_impedance = 50.0, name = nothing)
        z0 = Float64(reference_impedance)
        isfinite(z0)&&z0 > 0||throw(ArgumentError("port reference impedance must be finite and positive"))
        return new(positive, negative, z0, name === nothing ? nothing : Symbol(name))
    end
end
...
function port_response(c, frequency_specification::Union{Pair, AbstractVector}; ports, points = 100, scale = :log, temperature = 300.0, kw...)
    ...
    return NetworkResult(_snapshot_compiled(cc), Float64.(fs), deepcopy(port_list), z, stats)
end
...
function _z0_matrix(ports)
    return Matrix(Diagonal(ComplexF64[port.reference_impedance for port in ports]))
end
...
function renormalize(result::NetworkResult, reference_impedances)
    ...
    return NetworkResult(result.compiled, copy(result.frequencies), ports, copy(result.z), copy(result.stats))
end

provenance(result::NetworkResult) = Dict(
    :amber_version => v"0.1.0", :topology_fingerprint => result.compiled.fingerprint,
