...
function periodic_steady_state(c;period,saveat=Float64(period)/200,max_step=nothing,method=:bdf2,event_mode=nothing,
    ...
    autonomous&&event_mode!==nothing&&throw(AnalysisValidationError(
    ...
    for iteration in 1:maxiters
        ...
        elseif autonomous&&phase_reference===nothing
            1<=phase_index<=cc.n||throw(AnalysisValidationError(
                "autonomous PSS phase index is outside the state vector"))
            phase_reference=state[phase_index]
        end
        scale=max(norm(state,Inf),norm(endpoint,Inf),1.)
        phase_residual=autonomous ? state[phase_index]-phase_reference : 0.
    ...
    if autonomous&&converged
        ...
        neutral_distance>1e-3&&push!(warnings,
            "autonomous PSS has no well-isolated neutral Floquet multiplier")
    end
    stats=_finalize_stats!(Dict{Symbol,Any}(:converged=>converged,
        :iterations=>iteration_count,:residual_norm=>residual_norm,
        :temperature=>Float64(temperature),:autonomous=>autonomous,
        :monodromy_method=>effective_monodromy_method,:warnings=>warnings);partial=true)
    PSSResult(cycle,period,residual_norm,iteration_count,monodromy,multipliers,
...
simulate(c,analysis::PeriodicSteadyState)=periodic_steady_state(c;
