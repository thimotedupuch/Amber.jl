...
function _draw_white_noise(rng, sources, groups, frequency, bias, time, timestep)
    ...
    for group in groups
        ...
        psds = [
            _noise_source_psd(sources[index], frequency, bias, time)
...
function _colored_noise_sequences(rng, sources, groups, count, timestep, low_frequency_cutoff, bias)
    ...
    for index in colored
        ...
        for bin in 2:(length(bins) - 1)
            ...
            psd = _noise_source_psd(source, frequency, bias, 0.0)
        ...
        reference_psd[source.id] =
            _noise_source_psd(source, low_frequency_cutoff, bias, 0.0)
    end
    return sequences, reference_psd
end
...
function transient_noise(
        c, interval::Pair; timestep, saveat = timestep, seed,
        temperature = 300.0, initial = nothing, event_mode = nothing, reltol = nothing,
        abstol = nothing, maxiters = nothing,
        solver = SolverOptions(reltol = 1.0e-6, current_abstol = 1.0e-9, voltage_abstol = 1.0e-9, state_abstol = 1.0e-9, max_newton_iterations = 120),
    ...
    isapprox(t0 + step_count * timestep, t1; rtol = 64eps(Float64), atol = 64eps(Float64) * max(abs(t1), 1.0))||
    ...
    colored, colored_reference_psd = _colored_noise_sequences(
        rng, initial_sources,
        initial_groups, step_count,
        timestep, low_frequency_cutoff, state
    )
    complex_white = _complex_correlated_white_sequences(
        rng, initial_sources,
    ...
    for step in 1:step_count
        ...
        amplitudes = _draw_white_noise(
            rng, direct_sources, white_groups, nyquist,
        ...
        for source in sources
            ...
            current_psd = _noise_source_psd(
                source, low_frequency_cutoff, previous,
                times[step + 1]
            )
            modulation = initial_psd > 0 ? sqrt(current_psd / initial_psd) : 0.0
            forcing .+= real.(source.injection) .* colored[source.id][step] * modulation
        end
        state, iterations, converged = _newton(
            cc, state, previous, times[step + 1],
        ...
        if event_mode === :exact&&converged
            ...
            if any(!iszero, injected)
                ...
                state, event_iterations, converged = _newton(
                    cc, state, previous, times[step + 1],
                    inv(timestep); reltol, abstol, maxiters, temperature, forcing, workspace,
    ...
    stats = _finalize_stats!(
        Dict{Symbol, Any}(
            ...
            :source_metadata => [
                (
                    id = source.id, owner = source.owner,
    ...
    return SimulationResult(cc, analysis, times[saved_indices], values[:, saved_indices], stats)
end

simulate(c, analysis::TransientNoise) = transient_noise(
    c, analysis.interval;
