...
function _solver_residual(cc, workspace, state, derivative, t; mode = :time, source_scale = 1.0, gmin = 0.0, temperature = 300.0)
    return workspace === nothing ? residual(cc, state, derivative, t; mode, source_scale, gmin, temperature) :
...
function _update_converged(cc, z, delta, reltol, current_abstol, voltage_abstol, state_abstol)
    for index in eachindex(z)
        ...
        abs(delta[index]) <= absolute + reltol * max(abs(z[index]), abs(z[index] + delta[index]), 1.0)||return false
    end
    return true
end
...
function _new_factorization(system, solver::SuiteSparseLU)
    ...
    return solver.ordering === :amd ? lu(system; control) :
...
function _newton(
        cc, z0, previous, t, α; reltol = 1.0e-7, abstol = 1.0e-10, maxiters = 60,
        mode = :time, source_scale = 1.0, gmin = 0.0, temperature = 300.0, forcing = nothing, workspace = nothing,
        line_search_minimum = 1 / 256, voltage_abstol = max(abstol, 1.0e-9), state_abstol = abstol,
    ...
    factorization_key = linear_hierarchy ? (
            cc.topology, cc.parameters.matrix_fingerprint, Float64(α), mode, Float64(gmin),
            linear_solver,
        ) : nothing
    for it in 1:maxiters
        ...
        for column in 1:cc.n, pointer in nzrange(scaled_matrix, column)
            scaled_matrix.nzval[pointer] *= variable_scales[column]
        end
        ...
        for value_index in eachindex(system.nzval)
            system.nzval[value_index] *= inverse_row_norms[system.rowval[value_index]]
        end
        Δ = try
            rhs = workspace.rhs
        ...
        if !all(isfinite, Δ)
            history === nothing || push!(
                history, (
                    iteration = length(history) + 1,
                    local_iteration = it, residual_norm = norm(r, Inf), update_norm = Inf,
                    damping = 0.0, source_scale = Float64(source_scale), gmin = Float64(gmin),
                    converged = false,
                )
            )
            return z, it, false
        ...
        if residual_converged&&_update_converged(
                cc, z, Δ, reltol, abstol,
        ...
        @inbounds @simd for index in eachindex(z)
            z[index] += damping * Δ[index]
        end
        all(isfinite, z)||return z, it, false
    end
    return z, maxiters, false
end
