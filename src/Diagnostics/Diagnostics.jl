...
function _check_and_compile(design::CircuitDesign)
    ...
    design.root_ir.ground_net == 0 && push!(
        diagnostics, Diagnostic(
            :error,
    ...
    append!(
        bodies, (
            (_name(template.names, template.name), template.body, template.names)
    ...
    for (scope, body, names) in bodies
        for primitive in body.primitives
            ...
            terminal_count == contract.terminals || push!(
                diagnostics, Diagnostic(
                    :error,
    ...
    for batch in parameters.batches, device in eachindex(batch.locators)
        ...
        if kind === :voltage_source&&iszero(get(batch.parameters[device], :series_resistance, 0.0))
            ...
            push!(ideal_edges, (terminal(batch, 1, device), terminal(batch, 2, device), Float64(get(batch.parameters[device], :dc, 0.0)), path))
        elseif kind === :inductor
            instance_name, device_name = _locator_device_name(design, batch.locators[device])
            path = isempty(instance_name) ? device_name : string(instance_name, '.', device_name)
            push!(ideal_edges, (terminal(batch, 1, device), terminal(batch, 2, device), 0.0, path))
        end
    end
    ...
    for start in Int32(0):Int32(net_count)
        ...
        potentials[start] = 0.0; pending = Int32[start]
        while !isempty(pending)
            ...
            for (neighbor, offset, path) in voltage_graph[Int(net) + 1]
                ...
                elseif !isapprox(potentials[neighbor], expected; atol = 1.0e-12, rtol = 1.0e-12)
    ...
    return diagnostics, topology, parameters
end
