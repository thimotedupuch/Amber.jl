...
_contract(
    terminals; branch = false, states = Symbol[], dc_path = false, noise = false, observables = (:voltage, :current, :power),
...
function _describe_stamp!(emit, kind::Symbol, q, branch::Int32, states, control::Int32)
    return if kind in (:resistor, :conductance, :capacitor, :diode)
        for row in q[1:2], column in q[1:2]
            emit(row, column)
        end
    elseif kind in (:voltage_source, :inductor)
        for node in q[1:2]
            emit(node, branch); emit(branch, node)
        end
        emit(branch, branch)
    elseif kind === :vccs
        for row in q[3:4], column in q[1:2]
            emit(row, column)
        end
    elseif kind === :vcvs
        for node in q[3:4]
    ...
    elseif kind in (:npn, :nmos, :pmos)
        for row in q, column in q
            emit(row, column)
        end
    elseif kind === :switch
        for row in q[1:2], column in q
            emit(row, column)
        end
    elseif kind === :opamp
        state = first(states)
...
function _emit_stamp_positions!(emit, kind::Symbol, terminals, branch::Int32, states, control::Int32)
    for (row, column) in _DEVICE_STAMP_PLANS[kind].positions
        emit(
            _stamp_unknown(row, terminals, branch, states, control),
