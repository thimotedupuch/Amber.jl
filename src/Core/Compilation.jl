...
function compile(design::CircuitDesign)
    ...
    return CompiledCircuit(design, topology, parameters, fingerprint)
end
...
function _limited_exponential(argument)
    return if argument > 80.0
        expvalue = exp(80.0)
        expvalue * (1 + argument - 80.0) - 1, expvalue
    elseif argument < -80.0
        expvalue = exp(-80.0)
...
function _source_value(parameters, time, mode)
    mode === :matrix&&return 0.0
    mode === :dc&&return get(parameters, :dc, 0.0)
    waveform = get(parameters, :waveform, nothing)
    return waveform === nothing ? get(parameters, :dc, 0.0) : waveform(time)
end
...
function _switch_conductance(model, control)
    ...
    return inv(control >= model.threshold ? model.ron : model.roff)
end

function _switch_conductance_derivative(model, control)
    model isa SmoothSwitch||return 0.0
    transition = max(abs(model.transition), eps(Float64))
    argument = (control - model.threshold) / transition
    return 0.5 * (1 - tanh(argument)^2) / transition * (inv(model.ron) - inv(model.roff))
end

function residual(compiled::CompiledCircuit, state, derivative, time; mode = :time, source_scale = 1.0, gmin = 0.0, temperature = 300.0)
    workspace = SimulationWorkspace(compiled; scalar_type = eltype(state))
    return copy(residual!(workspace, compiled, state, derivative, time; mode, source_scale, gmin, temperature))
end
...
function ac_excitation(compiled::CompiledCircuit; source = nothing)
    ...
    for batch in compiled.parameters.batches
        ...
        for device in eachindex(batch.parameters)
            ...
            amplitude = ComplexF64(get(batch.parameters[device], :ac, 0.0))
...
function residual_jacobian(compiled::CompiledCircuit, state, previous, time, alpha; mode = :time, source_scale = 1.0, gmin = 0.0, temperature = 300.0)
    workspace = SimulationWorkspace(compiled; scalar_type = eltype(state))
    residual_values, jacobian_values = residual_jacobian!(
        workspace, compiled, state, previous, time, alpha;
        mode, source_scale, gmin, temperature
    )
    return copy(residual_values), copy(jacobian_values)
end
