...
function _storage_batch!(workspace, batch::PrimitiveBatch{Val{:capacitor}}, state, temperature)
    for device in eachindex(batch.parameters)
        ...
        _stamp_two_terminal!(
            workspace.storage, workspace.storage_jacobian.nzval, batch, device,
    ...
    for device in eachindex(batch.parameters)
        ...
        if model isa ChargeBasedMOSFET
            evaluated = _charge_mos_evaluate(
                model, kind, ntuple(j -> _workspace_value(state, nodes[j]), 4); temperature,
            ...
            for (a, b, c) in (
                    (2, 3, model.gate_source_capacitance),
...
function _storage_batches!(workspace, batches::Tuple, state, temperature)
    ...
    return _storage_batches!(workspace, Base.tail(batches), state, temperature)
end

"""Assemble stored charge/flux and its Jacobian in independent reusable buffers."""
function storage_jacobian!(workspace::SimulationWorkspace, cc::CompiledCircuit, state; temperature = 300.0)
    ...
    return workspace.storage, workspace.storage_jacobian
end

function _storage(cc, state; temperature = 300.0, workspace = nothing)
    ...
    return copy(workspace.storage)
end

function _static_dynamic_jacobians(cc, state, t = 0.0; temperature = 300.0, mode = :time)
    workspace = SimulationWorkspace(cc; scalar_type = eltype(state))
    return _static_dynamic_jacobians!(workspace, cc, state, t; temperature, mode)
end

function _static_dynamic_jacobians!(workspace, cc, state, t = 0.0; temperature = 300.0, mode = :time)
    _, g = residual_jacobian!(workspace, cc, state, state, t, 0.0; temperature, mode)
...
function _step_batch!(view, batch, state, zero_derivative, t, alpha, mode, source_scale, temperature)
    ...
    return (iszero(alpha) || !_has_storage(batch)) || _storage_batch!(view, batch, state, temperature)
end
...
function _step_batch!(view, batch::PrimitiveBatch{K}, state, zero_derivative, t, alpha, mode, source_scale, temperature) where {K <: Union{Val{:nmos}, Val{:pmos}}}
    for device in eachindex(batch.parameters)
        ...
        evaluated = _charge_mos_evaluate(
            model, _batch_kind(batch), ntuple(j -> _workspace_value(state, nodes[j]), 4);
...
function _step_batches!(view, batches::Tuple, args...)
    ...
    return _step_batches!(view, Base.tail(batches), args...)
end
...
function _constant_batches!(view, batches::Tuple, zero_state)
    ...
    if _linear_batch(batch)
        _assemble_batch!(view, batch, zero_state, zero_state, 0.0, 0.0, :matrix, 0.0, 300.0)
        _has_storage(batch) && _storage_batch!(view, batch, zero_state, 300.0)
    end
    return _constant_batches!(view, Base.tail(batches), zero_state)
end
...
function _ensure_constant_matrices!(workspace, cc)
    ...
    view = (;
        residual = workspace.residual, jacobian = workspace.constant_jacobian,
...
function _forcing_batches!(r, batches::Tuple, args...)
    ...
    return _forcing_batches!(r, Base.tail(batches), args...)
end

function _step_evaluate!(
        workspace, cc, state, qhistory, t, alpha, want_jacobian::Val{J};
...
function _bdf_storage_history(cc, previous, older, ratio, h, alpha; temperature = 300.0, workspace = nothing)
    ...
    return (((1 + ratio) / h) .* qprevious .- (ratio^2 / ((1 + ratio) * h)) .* qolder) ./ alpha
end
