...
function _encode_value(value)
    ...
    return Dict("type" => "struct", "name" => type_name, "fields" => fields)
end
...
function _decode_value(encoded, nodes, components, depth::Int = 0, max_depth::Int = 64)
    ...
    elseif kind == "struct"
        ...
        elseif type in (
                _PASSIVE_MODELS..., DebyeBranches, JunctionDiode,
...
function deserialize_circuit(text::AbstractString; max_bytes::Integer = 16 * 1024 * 1024, max_nodes::Integer = 1_000_000, max_components::Integer = 1_000_000, max_depth::Integer = 64)
    ...
    return try
        ...
        version == _CIRCUIT_SCHEMA_VERSION || throw(
...
function _hierarchy_encode(value)
    ...
    value isa NamedTuple && return Dict(
        "type" => "hierarchy_named_tuple", "entries" => [
...
function _hierarchy_decode(value, max_depth::Int, depth::Int = 0)
    ...
    return _decode_value(value, Dict{Int, AbstractNode}(), Dict{Any, Any}(), depth, max_depth)
end

function _segment_snapshot(segment::NameSegment, names::NameTable)
    return Dict("base" => _name(names, segment.base), "index" => _hierarchy_encode(segment.index))
end

function _segment_restore(snapshot, names::_NameBuilder, max_depth)
    return NameSegment(_intern!(names, snapshot["base"]), _hierarchy_decode(snapshot["index"], max_depth))
end
...
function _observation_snapshot(observation)
    ...
    if value isa BuilderNet || value isa BuilderPrimitive
        return Dict(
            "name" => observation.name === nothing ? "" : String(observation.name),
...
function _restore_body(snapshot, names::_NameBuilder, owner::UInt64, generation::UInt32, max_depth)
    ...
    return TemplateIR(net_names, Int32(snapshot["ground_net"]), terminal_data, primitives, observations)
end
...
function _restore_template(snapshot, max_depth)
    ...
    return SubcircuitTemplate(name, ports, parameters, body, _parse128(snapshot["structural_fingerprint"]), NameTable(copy(names.strings)))
end
...
function _deserialize_design(snapshot; max_nodes, max_components, max_depth)
    ...
    return CircuitDesign(
        name, TemplateRegistry(templates), InstanceGraph(records, connections, parameters),
