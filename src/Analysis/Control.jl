...
function _unit_source_excitation(cc, name)
    ...
    if kind === :voltage_source
        excitation[Int(batch.branch_unknowns[device])] = 1.0
    else
...
function _linear_output_selector(cc, observable::Observable)
    ...
    elseif observable.kind in (:state, :current)
        ...
        if observable.kind === :state
            ...
            selector[Int(batch.state_unknowns[device][state_index])] = 1.0; return selector
        elseif batch isa PrimitiveBatch&&batch.branch_unknowns[device] != 0
            selector[Int(batch.branch_unknowns[device])] = 1.0; return selector
...
function linearize(c; inputs, outputs, bias = nothing, temperature = 300.0, kw...)
    cc = compile(c)
    ...
    return LinearizedModel(Matrix(E), -Matrix(G), Matrix(B), Matrix(output_matrix), D, input_names, output_observables, Float64.(point), _snapshot_compiled(cc), stats)
end
...
function frequency_response(model::LinearizedModel, frequency_specification::Union{Pair, AbstractVector}; points = 100, scale = :log)
    ...
    return LinearFrequencyResponse(Float64.(fs), values, copy(model.inputs), copy(model.outputs), Dict{Symbol, Any}(:warnings => String[]))
end
...
function _generalized_values(model::LinearizedModel)
    return eigvals(model.A, model.E)
end
...
function transmission_zeros(model::LinearizedModel; input = 1, output = 1)
    ...
    return filter(isfinite, values)
end

function _siso_values(response::LinearFrequencyResponse; input = 1, output = 1)
    return vec(response.values[output, input, :])
end
gain_crossovers(response::LinearFrequencyResponse; input = 1, output = 1) = crossings(response.frequencies, db20(_siso_values(response; input, output)); level = 0.0)
phase_crossovers(response::LinearFrequencyResponse; input = 1, output = 1) = crossings(response.frequencies, rad2deg.(phase(_siso_values(response; input, output); unwrap = true)); level = -180.0)
...
function _interpolate_response(response::LinearFrequencyResponse, frequency; input = 1, output = 1)
    ...
    phases = phase(values; unwrap = true)
    return magnitude * cis(phases[index - 1] + fraction * (phases[index] - phases[index - 1]))
end
...
function stability_margins(response::LinearFrequencyResponse; input = 1, output = 1)
    ...
    return StabilityMargins(gm, pm, gc, pc)
end
...
function _time_response(model::LinearizedModel, interval::Pair; saveat, input_index = 1, impulse = false)
    ...
    return TimeResponse(Float64.(times), values, copy(model.inputs), copy(model.outputs), Dict{Symbol, Any}(:method => :bdf1, :warnings => String[]))
end
...
function rise_time(result::TimeResponse; input = 1, output = 1, levels = (0.1, 0.9))
    ...
    return _first_crossing(result.axis, values, high) - _first_crossing(result.axis, values, low)
end
function settling_time(result::TimeResponse; input = 1, output = 1, tolerance = 0.02)
    values = _time_trace(result; input, output); final = last(values); bound = tolerance * max(abs(final - first(values)), eps())
    last_outside = findlast(value -> abs(value - final) > bound, values)
    return last_outside === nothing ? first(result.axis) : last_outside == length(values) ? Inf : result.axis[last_outside + 1]
end
...
steady_state_error(result::TimeResponse; input = 1, output = 1, target = 1.0) = target - last(_time_trace(result; input, output))
...
function root_locus(model::LinearizedModel, gains; input = 1, output = 1)
    ...
    return [filter(isfinite, eigvals(model.A - gain * model.B[:, input:input] * model.output_matrix[output:output, :], model.E)) for gain in gains]
end
