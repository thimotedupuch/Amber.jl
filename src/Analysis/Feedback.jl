...
function _ideal_loop_wire(cc,name)
    ...
    iszero(get(parameters,:dc,0.))&&iszero(get(parameters,:series_resistance,0.))||
        throw(ArgumentError("the loop wire must be an ideal zero-DC voltage source"))
    Int(batch.terminals[1][device]),Int(batch.terminals[2][device]),Int(batch.branch_unknowns[device])
end
...
function loop_gain(circuit,frequency_specification::Union{Pair,AbstractVector};probe::AbstractLoopProbe,points=100,scale=:log,temperature=300.,kw...)
    ...
    probe.sign in (-1.,1.)||throw(ArgumentError("loop probe sign must be -1 or 1"))
    cc=compile(circuit)
    ...
    excitation[branch,1]=1. # unit series voltage
    p>0&&(excitation[p,2]+=1.) # unit current into the joined port
    reference>0&&(excitation[reference,2]-=1.)
    values=ComplexF64[]
    for frequency in fs
        response=_solve_linear(g+im*2π*frequency*c,excitation,
            "loop-gain matrix is singular at $(frequency) Hz")
        voltage(column)=(p==0 ? 0. : response[p,column])-
            (reference==0 ? 0. : response[reference,column])
    ...
    stats=Dict{Symbol,Any}(:bias_preserved=>true,:source=>probe.source,
