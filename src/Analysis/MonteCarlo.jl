...
struct Gaussian <: AbstractVariation
    ...
    function Gaussian(mean::Real, sigma::Real)
        ...
        return new(Float64(mean), Float64(sigma))
    end
end
...
struct LogNormal <: AbstractVariation
    ...
    function LogNormal(logmean::Real, logsigma::Real)
        ...
        return new(Float64(logmean), Float64(logsigma))
    end
end
...
struct UniformVariation <: AbstractVariation
    ...
    function UniformVariation(low::Real, high::Real)
        ...
        return new(Float64(low), Float64(high))
    end
end
...
struct CorrelatedVariation
    ...
    function CorrelatedVariation(paths, means, covariance)
        ...
        return new(normalized_paths, normalized_means, matrix, factor)
    end
end
...
failure_rate(result::MonteCarloResult) = isempty(result.values) ? 0.0 : count(!, result.converged) / length(result.values)
...
function Statistics.mean(result::MonteCarloResult)
    values = successful(result); isempty(values)&&throw(ArgumentError("Monte Carlo result has no successful samples"))
    return Statistics.mean(values)
end
function Statistics.std(result::MonteCarloResult; kw...)
    values = successful(result); isempty(values)&&throw(ArgumentError("Monte Carlo result has no successful samples"))
    return Statistics.std(values; kw...)
end
function Statistics.quantile(result::MonteCarloResult, p; kw...)
    values = successful(result); isempty(values)&&throw(ArgumentError("Monte Carlo result has no successful samples"))
    return Statistics.quantile(values, p; kw...)
end
function _normal_quantile_for_level(level)
    ...
    p = 0.5 + level / 2; a = 0.147; y = 2p - 1; logarithm = log(1 - y^2)
    return sqrt(2) * sign(y) * sqrt(sqrt((2 / (π * a) + logarithm / 2)^2 - logarithm / a) - (2 / (π * a) + logarithm / 2))
end
function confidence_interval(result::MonteCarloResult; level = 0.95)
    ...
    center = Statistics.mean(values); half = z * Statistics.std(values) / sqrt(length(values))
    return center - half => center + half
end
function yield_confidence_interval(result::MonteCarloResult, predicate::Function; level = 0.95)
    ...
    half = z / denominator * sqrt(estimate * (1 - estimate) / n + z^2 / (4n^2))
    ...
    for (name, components) in groups
        ...
        for component in components
            ...
            if saturation_path ∉ explicit_paths
                ...
                draws[saturation_path] = Float64(model.saturation_current) * exp(-delta_vbe / 0.025852)
            end
...
function monte_carlo(
        circuit; analysis = OperatingPoint(), samples::Integer = 1000, seed::Integer = 0,
    ...
    if parallel&&samples > 1
        Threads.@threads for sample in 1:samples
            run_sample(sample)
        end
    else
    ...
    metadata = Dict{Symbol, Any}(
        :seed => seed, :samples => samples, :circuit_fingerprint => base.fingerprint,
...
function replay_sample(result::MonteCarloResult, circuit, index::Integer; metric = identity)
    ...
    return metric(simulation)
end

function provenance(result::MonteCarloResult)
    return Dict(
        :amber_version => v"0.1.0", :analysis => string(typeof(result.analysis)), :monte_carlo => copy(result.metadata),
...
function report(result::MonteCarloResult)
    return EngineeringReport(
        Dict(
            :analysis => string(typeof(result.analysis)), :samples => length(result.values),
...
function _encode_analysis(analysis::OperatingPoint)
    return Dict("kind" => "operating_point", "temperature" => analysis.temperature, "solver" => _encode_value(analysis.solver))
end
function _encode_analysis(analysis::Transient)
    return Dict(
        "kind" => "transient", "interval" => [first(analysis.interval), last(analysis.interval)],
...
function _encode_analysis(analysis::SmallSignal)
    return Dict(
        "kind" => "small_signal", "frequency_grid" => _encode_value(analysis.frequencies),
...
function _encode_analysis(analysis::PeriodicSteadyState)
    ...
    return Dict(
        "kind" => "periodic_steady_state", "options" => _encode_value(
...
function _decode_analysis(encoded)
    ...
    kind == "operating_point"&&return OperatingPoint(temperature = get(encoded, "temperature", 300.0), solver = _decode_setting(encoded, "solver", SolverOptions()))
    if kind == "transient"
        interval = Float64(encoded["interval"][1]) => Float64(encoded["interval"][2])
        return Transient(
            interval; saveat = _decode_value(encoded["saveat"], Dict(), Dict()), max_step = _decode_value(encoded["max_step"], Dict(), Dict()),
    ...
    elseif kind == "transient_noise"
        ...
        return TransientNoise(
            ...
            solver = _decode_setting(encoded, "solver", SolverOptions(reltol = 1.0e-6, current_abstol = 1.0e-9, voltage_abstol = 1.0e-9, state_abstol = 1.0e-9, max_newton_iterations = 120))
        )
...
function serialize_monte_carlo(result::MonteCarloResult)
    snapshot = Dict{String, Any}(
        "schema" => "amber-monte-carlo", "schema_version" => 2,
        ...
        "converged" => collect(result.converged), "failures" => [
            Dict(
                "sample" => failure.sample, "seed" => string(failure.seed),
                "exception_type" => String(failure.exception_type), "message" => failure.message
            ) for failure in result.failures
        ],
        "metadata" => _encode_value(result.metadata)
    )
    io = IOBuffer(); TOML.print(io, snapshot; sorted = true)
...
function deserialize_monte_carlo(text::AbstractString; max_bytes = 64 * 1024 * 1024, max_samples = 10_000_000)
    ...
    snapshot = try
        ...
        throw(CircuitSerializationError("invalid Monte Carlo serialization: $(sprint(showerror, error))"))
    end
    ...
    return MonteCarloResult(_decode_analysis(snapshot["analysis"]), values, parameters, seeds, converged, failures, metadata)
end
