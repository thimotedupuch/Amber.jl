...
Base.@kwdef struct SolverOptions
    ...
    voltage_abstol::Float64 = 1.0e-9
    current_abstol::Float64 = 1.0e-12
    state_abstol::Float64 = 1.0e-10
...
function _validate_solver_options(options::SolverOptions)
    ...
    for (name, value) in (
            ("voltage_abstol", options.voltage_abstol),
...
function _solver_values(
        options::SolverOptions; reltol = nothing, abstol = nothing,
    ...
    resolved_depth >= 0 ||
        throw(AnalysisValidationError("continuation_maxdepth must be non-negative"))
    return resolved_reltol, resolved_abstol, resolved_maxiters, resolved_depth
end

function _effective_solver(
        options::SolverOptions; reltol = nothing, abstol = nothing,
        maxiters = nothing, continuation_maxdepth = nothing
    )
    r, a, m, d = _solver_values(options; reltol, abstol, maxiters, continuation_maxdepth)
    return SolverOptions(
        reltol = r, current_abstol = a,
...
Base.@kwdef struct IntegrationOptions
    ...
    voltage_abstol::Float64 = 1.0e-7
    current_abstol::Float64 = 1.0e-10
    state_abstol::Float64 = 1.0e-9
    max_steps::Int = 100_000
    min_step::Float64 = 0.0
end
...
Base.@kwdef struct TransientNoise <: AbstractAnalysis
    ...
    solver::SolverOptions = SolverOptions(reltol = 1.0e-6, current_abstol = 1.0e-9, voltage_abstol = 1.0e-9, state_abstol = 1.0e-9, max_newton_iterations = 120)
end
...
function _validate_transient(interval; saveat = nothing, max_step = nothing, method = :bdf2, event_mode = nothing, reltol = 1.0e-6, abstol = 1.0e-9, maxiters = 120)
    ...
    return maxiters > 0||throw(AnalysisValidationError("maxiters must be positive"))
end
...
function _validate_frequency_range(range, points, scale)
    ...
    return points >= 1||throw(AnalysisValidationError("frequency point count must be positive"))
end
...
function SmallSignal(range::Pair; points = 100, scale = :log, kw...)
    ...
    return SmallSignal(frequencies; kw...)
end
...
function _override_pairs(overrides)
    ...
    return [Symbol(first(item)) => last(item) for item in overrides]
end
