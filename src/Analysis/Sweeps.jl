...
failure_rate(result::SweepResult) = isempty(result.converged) ? 0.0 : count(!, result.converged) / length(result.converged)
...
function sweep(c, p::Pair; analysis = OperatingPoint(), metric = identity)
    ...
    for (index, value) in enumerate(parameter_values)
        try
            ...
            get(simulation.stats, :converged, false)||throw(
                ConvergenceError(
    ...
    metadata = Dict{Symbol, Any}(
        :selector => selector, :points => length(parameter_values),
        :successful_points => count(converged), :failed_points => count(!, converged),
        :circuit_fingerprint => cc.fingerprint
    )
    return SweepResult(selector, parameter_values, metrics, simulations, converged, failures, analysis, metadata)
end

provenance(result::SweepResult) = Dict(
    :amber_version => v"0.1.0", :analysis => string(typeof(result.analysis)),
    :sweep => copy(result.metadata), :failures => copy(result.failures), :unit_system => :SI
)
report(result::SweepResult) = EngineeringReport(
    Dict(
        :analysis => string(typeof(result.analysis)), :selector => result.selector,
