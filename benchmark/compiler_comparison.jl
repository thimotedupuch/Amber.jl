...
function measure(f;repeats=30)
    ...
    for _ in 1:repeats
        result=@timed f()
        if result.time<elapsed; elapsed=result.time; bytes=result.bytes end
    end
    Dict("seconds"=>elapsed,"bytes"=>bytes)
end
function compare(cells,mos)
    d=design(cells,mos); cc=compile(d); ws=SimulationWorkspace(cc)
    x=fill(.4,cc.n); q=Amber._storage(cc,x .- .001); alpha=1e6
    values=Dict{String,Any}(
        "compile"=>measure(()->compile(d);repeats=5),
        "step_jacobian"=>measure(()->Amber._step_residual_jacobian!(ws,cc,x,q,0.,alpha)),
        "parameter_update"=>measure(()->with_parameters(cc,"cell[1:$(cells)].R.value"=>2e3)),
        "ac"=>measure(()->small_signal(cc,10. .^ range(1,6;length=40));repeats=3))
    if isdefined(Amber,:_step_residual!)
        values["line_search_residual"]=measure(()->Amber._step_residual!(ws,cc,x,q,0.,alpha))
        handle=parameter_handle(cc,"cell[1:$(cells)].R.value")
        values["resolved_update"]=measure(()->with_parameters(cc,handle=>2e3))
    else
...
TOML.print(stdout,Dict("cells"=>cells,"julia_version"=>string(VERSION),"threads"=>Threads.nthreads(),
    "passive"=>compare(cells,false),"cmos"=>compare(cells,true));sorted=true)
