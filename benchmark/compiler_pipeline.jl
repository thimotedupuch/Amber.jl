...
@subcircuit PipelineCell(input,output,reference; R=1e3,C=1e-12) begin
...
function pipeline_design(cells;nonlinear=false,behavioral=false)
    ...
    add!(builder,voltage_source(nodes[0],gnd;dc=.8,ac=1.);name=:Source)
    instances!(builder,PipelineCell,1:cells;
        name=i->(:cell,i),connections=i->(input=nodes[i-1],output=nodes[i],reference=gnd))
    if nonlinear
        model=ChargeBasedMOSFET(width=2e-6,length=1e-6)
        for i in 1:cells
    ...
    if behavioral
        ...
        for (i,(law,slope)) in enumerate(zip((sin,tanh,exp,identity),
                (cos,x->1-tanh(x)^2,exp,x->1.)))
            i>cells && break
            add!(builder,behavioral_current_source(((nodes[i],gnd),),nodes[i],gnd;
                current=(v,t)->1e-6*law(v[1]),
                gradient=(v,t)->(1e-6*slope(v[1]),0.,0.,0.));name=(:B,i))
        end
    end
    finish(builder)
end
...
function sample_time(f;repeats=10)
    ...
    Dict("seconds"=>best.time,"allocated_bytes"=>best.bytes)
end
...
function pipeline_measure(cells;nonlinear=false,behavioral=false)
    ...
    x=fill(.4,cc.n); previous=x .- .001; alpha=1e6
    history=Amber._storage(cc,previous)
    first_evaluation=@timed Amber._step_residual_jacobian!(ws,cc,x,history,0.,alpha)
    ...
    result=Dict{String,Any}(
        ...
        "step_residual_jacobian"=>sample_time(()->Amber._step_residual_jacobian!(ws,cc,x,history,0.,alpha)),
        "step_residual"=>sample_time(()->Amber._step_residual!(ws,cc,x,history,0.,alpha)),
        "text_parameter_update"=>sample_time(()->with_parameters(cc,"cell[1:$(cells)].R1.value"=>2e3)),
        "resolved_parameter_update"=>sample_time(()->with_parameters(cc,handle=>2e3)),
        "structural_analysis"=>sample_time(()->structural_analysis(cc)),
        "workspace_retained_bytes"=>Base.summarysize(ws))
    if !behavioral
        fs=10. .^ range(1,6;length=40)
...
results=Dict("cells"=>cells,"julia_version"=>string(VERSION),"threads"=>Threads.nthreads(),
