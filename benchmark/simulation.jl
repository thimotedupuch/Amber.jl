...
@subcircuit SimulationCell(input,output,reference; R=1e3,C=1e-9) begin
...
function simulation_design(cells)
    ...
    add!(builder,voltage_source(nodes[0],reference;dc=1.,waveform=Step(at=0.));name=:Source)
    instances!(builder,SimulationCell,1:cells;
        name=i->(:cell,i),
...
function measure(cells,steps)
    ...
    transient_run=@timed transient(compiled,0.0 => 1e-4;initial=:discharged,
        saveat=1e-4/steps,max_step=1e-4/steps)
    transient_run.value.stats[:converged]||error("benchmark transient failed")
    result=Dict{String,Any}("cells"=>cells,"steps"=>steps,"unknowns"=>compiled.n,
    ...
    for (name,timing) in (("construction",construction),("compilation",compilation),
