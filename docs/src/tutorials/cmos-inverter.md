...
nmodel=Level1MOSFET(threshold_voltage=.7V,
    transconductance=2mA/V^2,channel_length_modulation=.03/V,
    body_effect=.35sqrt(V),surface_potential=.35V,
    gate_source_capacitance=8pF,gate_drain_capacitance=2pF)
pmodel=Level1MOSFET(threshold_voltage=.7V,
...
```@example cmos_inverter
@circuit CMOSInverter(;input_voltage=0V,waveform=nothing,
        load_capacitance=20pF) begin
...
outputs=[voltage(operating_point(CMOSInverter(input_voltage=value)),:output)[1]
    for value in inputs]
